...
class OperatorSmarts:    
    ...
    enthalpy_correction: typing.Optional[float] = None # add a correction term for enthalpy filter
    ring_issue: typing.Optional[bool] = False # if True, should check stoichiometry (including H) and connectivity (example: breaking 2 bonds and form a fragment, will have problem if the two fragments are connected by other parts. example2: cope rearrangements, C1 and C6 shouldn't connect, will have problem if they are) of reactants and products in a filter; ring issue could also cause RDkit error by breaking aromatic molecules, if a breaking group is directly on a aromatic molecule, consider use kekulize_flag as well (example Transesterification). 
    kekulize_flag: typing.Optional[bool] = False # if need to break aromatic bonds, use engine.op.rdkit(smarts, kekulize=True) to kekulize before running the SMARTS template matching
    Retro_Not_Aromatic: typing.Optional[bool] = False # only useful in retro list. if true, the forward reaction don't apply to aromatic molecules; should check if number of aromatic rings increase to avoid producing aromatic molecules.
    number_of_steps: typing.Optional[int] = 1 # optional for multi-step rxn operators, may be useful for enthalpy filter
    allowed_elements: typing.Optional[tuple[str,...]] = ("All", ) # can be used to specify the allowed elements in the reactants, and use a filter during expansion to filter out reactions if reactants contain other elements. For example, allowed_elements = ("C", "H") means this operator is intended only for hydrocarbons
    reaction_type: typing.Optional[str] = "Catalytic" # Catalytic for chemical reactions. the other option is Enzymatic, for bio rxn rules
...
#                   (1, 1),(1, 1)),
    ...
    OperatorSmarts("Alkane Dehydrogenation", 
                   "[C+0:1]=[C+0:2].[H][H]>>[*:1][*:2]",
                   (1, 1),(1, ),
                   kekulize_flag = True,
                   allowed_elements = ("C", "H")),    
    OperatorSmarts("Alkane Dehydrogenation 2", 
    ...
    OperatorSmarts("Hydrogenation of Alkene", 
                   "[C+0!H0:1][C+0!H0:2]>>[*:1]=[*:2].[H][H]",
                   (1, ),(1, 1),
                   Retro_Not_Aromatic = True),
    # Oxidative Cleavage of Alkenes  
    OperatorSmarts("Oxidative Cleavage of Alkenes", 
                   "[C;!$(*(=O)=O);+0:1]=[O+0:4].[C;!$(*(=O)=O);+0:2]=[O+0:5]>>[*:1]=[*:2].[*:4]=[*:5]",
                   (1, 1),(1, 1)),
    OperatorSmarts("Oxidative Cleavage of Alkenes, Intramolecular", 
                   "([C;!$(*(=O)=O);+0:1]=[O+0:4].[C;!$(*(=O)=O);+0:2]=[O+0:5])>>[*:1]=[*:2].[*:4]=[*:5]",
                   (1, ),(1, 1),
                   ring_issue = True,
                   Retro_Not_Aromatic = True),       
    # Olefin Metathesis
        # Cross Metathesis (CM) 
    OperatorSmarts("Olefin Cross Metathesis (CM)", 
    ...
    OperatorSmarts("Olefin Ring-Opening Metathesis (ROM) 1", 
...
#                   ( , ),( ,  )),          
    ...
    OperatorSmarts("Olefin Ring-Closing Metathesis (RCM)", 
                   "[C+0:1]=@[C+0:2].[C+0:3]=!@[C+0:4]>>([*:1]=[*:3].[*:2]=[*:4])", 
                   (1, 1),(1, ),
                   kekulize_flag = True),          
    ...
    OperatorSmarts("Reductive Cross-Coupling of Alkenes", 
                   "[C+0!H0:1][C+0:2]-!@[C+0:3][C+0!H0:4]>>[*:1]=[*:2].[*:3]=[*:4].[H][H]",
                   (1, ),(1, 1, 1),
                   Retro_Not_Aromatic = True),   
     
    # Epoxidation of Alkene
    OperatorSmarts("Epoxidation of Alkene", 
                   "[C+0:1]1[C+0:2][O+0:3]1>>[*:1]=[*:2].[*:3]=[O]",
                   (1, ),(1, 0.5),
                   Retro_Not_Aromatic = True),         
    # Hydration of Alkene, Ethers from Addition of Alcohols or Acids to Alkenes
    OperatorSmarts("Hydration of Alkene, Addition of Alcohols or Acids to Alkenes", 
                   "[C+0!H0:1][C!$(*=O)+0:2]-!@[O+0:3]>>[*:1]=[*:2].[*:3]",
                   (1, ),(1, 1),
                   Retro_Not_Aromatic = True),    
    OperatorSmarts("Addition of Alcohols or Acids to Alkenes, Intramolecular", 
                   "[C+0!H0:1][C!$(*=O)+0:2]-@[O+0:3][C+0:4]>>([*:1]=[*:2].[*:3][*:4])",
                   (1, ),(1, ),
                   Retro_Not_Aromatic = True),
    # Hydration of Alkenes, 2-step
    OperatorSmarts("Hydration of Alkenes, 2-step", 
                   "([C+0!H0:1][C+0:2][O+0H:5].[C+0!H0:3][C+0:4][O+0H])>>([*:1]=[*:2].[*:3]=[*:4]).[*:5]",
                   (1, ),(1, 2),
                   number_of_steps = 2,
                   Retro_Not_Aromatic = True),    
    # Hydrohalogenation of Alkenes 
    OperatorSmarts("Hydrohalogenation of Alkenes", 
                   "[C+0!H0:1][C+0:2][F,Cl,Br,I;+0:3]>>[*:1]=[*:2].[*:3]",
                   (1, ),(1, 1),
                   Retro_Not_Aromatic = True),          
    # Halogenation of Alkenes
    OperatorSmarts("Halogenation of Alkenes", 
    ...
    OperatorSmarts("Diels-Alder Reaction with Alkenes", 
                   "[C+0:1]1[C+0:2]=[C+0:3][C+0:4][C+0:6][C+0:5]1>>[*:1]=[*:2][*:3]=[*:4].[*:5]=[*:6]",
                   (1, ),(1, 1),
                   ring_issue = True), 
    OperatorSmarts("Diels-Alder Reaction with Alkenes, Intramolecular", 
                   "[C+0:1]1[C+0:2]=[C+0:3][C+0:4][C+0:6][C+0:5]1>>([*:1]=[*:2][*:3]=[*:4].[*:5]=[*:6])",
                   (1, ),(1, ),
                   ring_issue = True),  
    OperatorSmarts("Diels-Alder Reaction with Alkynes", 
                   "[C+0:1]1[C+0:2]=[C+0:3][C+0:4][C+0:6]=[C+0:5]1>>[*:1]=[*:2][*:3]=[*:4].[*:5]#[*:6]",
                   (1, ),(1, 1),
                   ring_issue = True),
    OperatorSmarts("Diels-Alder Reaction with Alkynes, Intramolecular", 
                   "[C+0:1]1[C+0:2]=[C+0:3][C+0:4][C+0:6]=[C+0:5]1>>([*:1]=[*:2][*:3]=[*:4].[*:5]#[*:6])",
                   (1, ),(1, ),
                   ring_issue = True),         
    # Oxo-Diels-Alder Reaction
    OperatorSmarts("Oxo-Diels-Alder Reaction", 
                   "[C+0:1]1[C+0:2]=[C+0:3][C+0:4][O+0:6][CX4!$(*(-O)-O)!$(*(-O)(-O)-O)+0:5]1>>[*:1]=[*:2][*:3]=[*:4].[*:5]=[*:6]",
                   (1, ),(1, 1),
                   ring_issue = True,
                   Retro_Not_Aromatic = True),     
    OperatorSmarts("Oxo-Diels-Alder Reaction, Intramolecular", 
                   "[C+0:1]1[C+0:2]=[C+0:3][C+0:4][O+0:6][CX4!$(*(-O)-O)!$(*(-O)(-O)-O)+0:5]1>>([*:1]=[*:2][*:3]=[*:4].[*:5]=[*:6])",
                   (1, ),(1, ),
                   ring_issue = True,
                   Retro_Not_Aromatic = True),    
    # Acrolein Diels-Alder Reaction
    OperatorSmarts("Acrolein Diels-Alder Reaction", 
                   "[C+0:1]1[C+0:2]=[C!$(*(-O)-O)+0:3][O+0:4][C+0:6][C+0:5]1>>[*:1]=[*:2][*:3]=[*:4].[*:5]=[*:6]",
                   (1, ),(1, 1),
                   ring_issue = True,
                   Retro_Not_Aromatic = True),     
    OperatorSmarts("Acrolein Diels-Alder Reaction, Intramolecular", 
                   "[C+0:1]1[C+0:2]=[C!$(*(-O)-O)+0:3][O+0:4][C+0:6][C+0:5]1>>([*:1]=[*:2][*:3]=[*:4].[*:5]=[*:6])",
                   (1, ),(1, ),
                   ring_issue = True,
                   Retro_Not_Aromatic = True), 
    # Halohydrin Formation
    OperatorSmarts("Halohydrin Formation", 
                   "[C+0:1]([O+0H:5])[C+0:2][F,Cl,Br,I;+0:3].[F,Cl,Br,I;+0H:4]>>[*:1]=[*:2].[*:3][*:4].[*:5]",
                   (1, 1),(1, 1, 1),
                   Retro_Not_Aromatic = True), 
    # Diol Formation by Oxidation
    OperatorSmarts("Diol Formation by Oxidation", 
                   "[C+0:1]([O+0H:3])[C+0:2][O+0H:4]>>[*:1]=[*:2].[*:3]=[O].[*:4]",
                   (1, ),(1, 0.5, 1),
                   Retro_Not_Aromatic = True),
    # Conjugated Dienes 1,4 Addition
    OperatorSmarts("Conjugated Dienes 1,4 Addition with Hydrogen Halide", 
                   "[F,Cl,Br,I;+0:5][C+0:1][C+0:2]=[C+0:3][C+0!H0:4]>>[*:1]=[*:2][*:3]=[*:4].[*:5]",
                   (1, ),(1, 1),
                   Retro_Not_Aromatic = True),
    OperatorSmarts("Conjugated Dienes 1,4 Addition with Halogens", 
    ...
    OperatorSmarts("Claisen Rearrangement, Cope Rearrangements", 
    ...
    OperatorSmarts("Aromatic Claisen Rearrangement 1", 
                   "[c+0:1]1([C+0:10][C+0:9]=[C+0:8]):[c+0:2]:[c+0:3]:[c+0:4]:[c+0:5]:[c+0:6]:1-[O+0H:7]>>[*:1]1:[*:2]:[*:3]:[*:4]:[*:5]:[*:6]:1-[*:7][*:8][*:9]=[*:10]",
                   (1, ),(1, ),
                   Retro_Not_Aromatic = True), 
    OperatorSmarts("Aromatic Claisen Rearrangement with Ortho Substituents", 
    ...
    # Tsuji–Trost Reaction
     ...
     OperatorSmarts("Intramolecular Tsuji-Trost Reaction, Nu = Active Methylenes", 
                    "[C+0:1]=[C+0:2][C+0;!$(*[OH]);!$(*=O):3]-@[C+0:7]([C+0:5]=[O+0:6])[C+0:8]=[O+0:9].[F,Cl,Br,I,O$(*C=O);+0H:4]>>([*:1]=[*:2][*:3][*:4].[*:5](=[*:6])[*:7][*:8]=[*:9])",
                    (1, 1),(1, )), 
        # Intramolecular Tsuji-Trost Reaction, Nu = Enolates
     OperatorSmarts("Intramolecular Tsuji-Trost Reaction, Nu = Enolates", 
                    "[C+0:1]=[C+0:2][C+0;!$(*[OH]);!$(*=O):3]-@[C+0:7]([C+0:5])[C+0:8]=[O+0:9].[F,Cl,Br,I,O$(*C=O);+0H:4]>>([*:1]=[*:2][*:3][*:4].[*:5][*:7][*:8]=[*:9])", 
                    (1, 1),(1, )),
        # Intramolecular Tsuji-Trost Reaction, Nu = Amines
     OperatorSmarts("Intramolecular Tsuji-Trost Reaction, Nu = Amines", 
                    "[C+0:1]=[C+0:2][C+0;!$(*[OH]);!$(*=O):3]-@[N+0:7][C,c;+0:8].[F,Cl,Br,I,O$(*C=O);+0H:4]>>([*:1]=[*:2][*:3][*:4].[*:7][*:8])",
                    (1, 1),(1, )),   
        # Intramolecular Tsuji-Trost Reaction, Nu = Phenols
     OperatorSmarts("Intramolecular Tsuji-Trost Reaction, Nu = Phenols", 
                    "[C+0:1]=[C+0:2][C+0;!$(*[OH]);!$(*=O):3]-@[O+0:7][c+0:8].[F,Cl,Br,I,O$(*C=O);+0H:4]>>([*:1]=[*:2][*:3][*:4].[*:7][*:8])", 
                    (1, 1),(1, )),
    # Oxychlorination
    OperatorSmarts("Oxychlorination", 
                   "[C+0:1]([F,Cl,Br,I;+0:3])[C+0:2][F,Cl,Br,I;+0].[O+0H2:4]>>[*:1]=[*:2].[*:3].[*:4]=[O]",
                   (1, 1),(1, 2, 0.5),
                   Retro_Not_Aromatic = True), 
    # Acetoxylation of Alkenes
    OperatorSmarts("Acetoxylation of Alkenes", 
                   "[C+0:3](=[O+0:4])[O+0:5]-!@[C+0:1]=[C+0:2].[O+0H2:6]>>[*:1]=[*:2].[*:3](=[*:4])[*:5].[*:6]=[O]",
                   (1, 1),(1, 1, 0.5)),          
    OperatorSmarts("Acetoxylation of Alkenes, Intramolecular", 
                   "[C+0:3](=[O+0:4])[O+0:5]-@[C+0:1]=[C+0:2].[O+0H2:6]>>([*:1]=[*:2].[*:3](=[*:4])[*:5]).[*:6]=[O]",
                   (1, 1),(1, 0.5)), # does not work for O=C1OC=CC=C1 unless add kekulize_flag
    # Oxidation of Propene
    OperatorSmarts("Oxidation of Propene", 
    ...
    OperatorSmarts("Halogen Addition of Alkynes", 
                   "[C+0:1]([F,Cl,Br,I;+0:3])=[C+0:2][F,Cl,Br,I;+0:4]>>[*:1]#[*:2].[*:3][*:4]", 
                   (1, ),(1, 1)), 
    # Hydrogen Halides Addition of Alkynes
    OperatorSmarts("Hydrogen Halides Addition of Alkynes", 
                   "[C+0:1]([F,Cl,Br,I;+0:3])=[C+0!H0:2]>>[*:1]#[*:2].[*:3]", 
                   (1, ),(1, 1)),     
    # Hydration of Alkynes
    OperatorSmarts("Hydration of Alkynes", 
                   "[C+0:1]([O+0H:3])=[C+0!H0:2]>>[*:1]#[*:2].[*:3]",
                   (1, ),(1, 1)),
    # Hydrogenation of Alkynes
    OperatorSmarts("Hydrogenation of Alkynes", 
                   "[C+0!H0:1]=[C+0!H0:2]>>[*:1]#[*:2].[H][H]",
                   (1, ),(1, 1)), 
    OperatorSmarts("Hydrogenation of Alkynes to Alkanes", 
                   "[C;H2,H3;+0:1][C;H2,H3;+0:2]>>[*:1]#[*:2].[H][H]",
                   (1, ),(1, 2)),    
    # Gentle Alkyne Oxidation 
    OperatorSmarts("Gentle Alkyne Oxidation", 
                   "[C+0:1](=[O+0:4])[C+0:2]=[O+0:5]>>[*:1]#[*:2].[*:4]=[*:5]",
                   (1, ),(1, 1)),    
    # Oxidative Cleavage of Alkynes 
    OperatorSmarts("Oxidative Cleavage of Alkynes", 
                   "[*:6][C+0:1](=[O+0:3])[O+0H:4].[*:7][C+0:2](=[O+0:5])[O+0H]>>[*:6][*:1]#[*:2][*:7].[*:3].[*:4]=[*:5]",
                   (1, 1),(1, 1, 1.5)),
    OperatorSmarts("Oxidative Cleavage of Alkynes Intramolecular", 
                   "([*:6][C+0:1](=[O+0:3])[O+0H:4].[*:7][C+0:2](=[O+0:5])[O+0H])>>[*:6][*:1]#[*:2][*:7].[*:3].[*:4]=[*:5]",
                   (1, ),(1, 1, 1.5)),
    OperatorSmarts("Oxidative Cleavage of Terminal Alkynes", 
                   "[*:6][C+0:1](=[O+0:4])[O+0H:5].[O]=[C+0:2]=[O]>>[*:6][*:1]#[*:2].[*:4]=[*:5]",
                   (1, 1),(1, 2)),
    # Alkynes by Dehydrohalogenation
    OperatorSmarts("Alkynes by Dehydrohalogenation of Alkanes", 
                   "[C+0:1]#[C+0:3].[F,Cl,Br,I;+0H:2]>>[*:1]([*:2])[*:3]([*:2])",
                   (1, 2),(1, )), 
    OperatorSmarts("Alkynes by Dehydrohalogenation of Alkenes", 
    ...
    OperatorSmarts("Alkylation of Acetylide Anions with Methyl and Primary Haloalkanes", 
                   "[C+0:1]#[C+0:2]-!@[C;H2,H3;+0:4].[F,Cl,Br,I;+0H:3]>>[*:1]#[*:2].[*:3][*:4]",
                   (1, 1),(1, 1),
                   enthalpy_correction = -54.50),    # DFT by QE
    # Sonogashira Coupling, Acyl Sonogashira Coupling
    OperatorSmarts("Sonogashira Coupling", 
    ...
    OperatorSmarts("Addition of Alkynes to Aldehydes", 
                   "[C+0:1]#[C+0:2]-!@[C+0!H0:3][O+0H:4]>>[*:1]#[*:2].[*:3]=[*:4]",
                   (1, ),(1, 1)),        
    OperatorSmarts("Addition of Alkynes to Aldehydes, Intramolecular", 
                   "[C+0:1]#[C+0:2]-@[C+0!H0:3][O+0H:4]>>([*:1]#[*:2].[*:3]=[*:4])",
                   (1, ),(1, )),    
    OperatorSmarts("Addition of Alkynes to Activated Ketones", 
    ...
    OperatorSmarts("Alcohols React with Hydrogen Halides to form Haloalkanes, Carboxylic Acids Conversion to Acid Chlorides", 
                   "[C+0:1][F,Cl,Br,I;+0:3].[O+0H2:2]>>[*:1][*:2].[*:3]", 
                   (1, 1),(1, 1)),     
    # Haloalkane Hydrolysis, Acid Chlorides Hydrolysis, Haloarenes Hydrolysis
    OperatorSmarts("Haloalkane Hydrolysis, Acid Chlorides Hydrolysis, Haloarenes Hydrolysis", 
                   "[C,c;+0:1][O+0H:2].[F,Cl,Br,I;+0H:3]>>[*:1][*:3].[*:2]",
                   (1, 1),(1, 1)),                        
    # Wurtz Reaction, Coupling of Halides with Gilman Reagent                  may need enthalpy correction
    OperatorSmarts("Wurtz Reaction, Coupling of Halides with Gilman Reagent", 
                   "[C,c;+0:1]-!@[C,c;+0:3].[F,Cl,Br,I;+0:2][F,Cl,Br,I;+0:4]>>[*:1][*:2].[*:3][*:4]",
                   (1, 1),(1, 1)),      
    OperatorSmarts("Wurtz Reaction, Coupling of Halides with Gilman Reagent, Intramolecular", 
                   "[C,c;+0:1]-@[C,c;+0:3].[F,Cl,Br,I;+0:2][F,Cl,Br,I;+0:4]>>([*:1][*:2].[*:3][*:4])",
                   (1, 1),(1, ),
                   kekulize_flag = True),     
    # Dehydrohalogenation
    OperatorSmarts("Dehydrohalogenation", 
                   "[C+0:1]=[C+0:3].[F,Cl,Br,I;+0H:2]>>[*:1]([*:2])[*:3]",
                   (1, 1),(1, ),
                   kekulize_flag = True),
    # Aromatic Halogenation
    OperatorSmarts("Aromatic Halogenation", 
                   "[c+0:1][F,Cl,Br,I;+0:2].[F,Cl,Br,I;+0H:3]>>[*:1].[*:2][*:3]",
                   (1, 1),(1, 1)), 
    # Friedel–Crafts Reaction        does not work for alkenyl/alkynyl halides
    OperatorSmarts("Friedel–Crafts Reaction", 
                   "[c+0:1]-!@[CX4,C$(*=O);+0:2].[F,Cl,Br,I;+0H:3]>>[*:1].[*:2][*:3]",
                   (1, 1),(1, 1)), 
    OperatorSmarts("Friedel–Crafts Reaction, Intramolecular", 
                   "[c+0:1]-@[CX4,C$(*=O);+0:2].[F,Cl,Br,I;+0H:3]>>([*:1].[*:2][*:3])",
                   (1, 1),(1, )),      # might use kekulize flag for products like O=c1occc2ccccc12? but would need to rewrite
    OperatorSmarts("Friedel–Crafts Acylation with Carboxylic Acids",    # doi.org/10.1021/ol800752v
                   "[c+0:1]-!@[C$(*=O)+0:2].[O+0H2:3]>>[*:1].[*:2][*:3]",
                   (1, 1),(1, 1)),
    OperatorSmarts("Friedel–Crafts Acylation with Carboxylic Acids, Intramolecular",    
                   "[c+0:1]-@[C$(*=O)+0:2].[O+0H2:3]>>([*:1].[*:2][*:3])",
                   (1, 1),(1, )),   
    OperatorSmarts("Friedel–Crafts Acylation with Acid Anhydrides", 
                   "[c+0:1]-!@[C$(*=O)+0:2].[O+0H:3][C$(*=O)+0:4]>>[*:1].[*:2][*:3][*:4]",
                   (1, 1),(1, 1)),
    OperatorSmarts("Friedel–Crafts Acylation with Acid Anhydrides, Intramolecular", 
                   "[c+0:1]-@[C$(*=O)+0:2].[O+0H:3][C$(*=O)+0:4]>>([*:1].[*:2][*:3][*:4])",
                   (1, 1),(1, )),
    OperatorSmarts("Friedel–Crafts Reaction with Alkenes", 
                   "[c+0:1]-!@[C+0:2][C+0!H0:3]>>[*:1].[*:2]=[*:3]",
                   (1, ),(1, 1),
                   Retro_Not_Aromatic = True), 
    OperatorSmarts("Friedel–Crafts Reaction with Alkenes, Intramolecular", 
                   "[c+0:1]-@[C+0:2][C+0!H0:3]>>([*:1].[*:2]=[*:3])",
                   (1, ),(1, ),
                   Retro_Not_Aromatic = True),    
    # Heck Reaction      doi.org/10.1039/C4CC00297K
    OperatorSmarts("Heck Reaction 1", 
                   "[C+0:1]=[C+0:2]-!@[c+0:3].[F,Cl,Br,I;+0H:4]>>[*:1]=[*:2].[*:3][*:4]",
                   (1, 1),(1, 1)),
    OperatorSmarts("Heck Reaction 2", 
                   "[c+0:5][C+0:1]=[C+0:2]-!@[C,c;+0:3].[F,Cl,Br,I;+0H:4]>>[*:5][*:1]=[*:2].[*:3][*:4]",
                   (1, 1),(1, 1)),
    OperatorSmarts("Heck Reaction 1, Intramolecular", 
                   "[C+0:1]=[C+0:2]-@[c;+0:3].[F,Cl,Br,I;+0H:4]>>([*:1]=[*:2].[*:3][*:4])",
                   (1, 1),(1, ),
                   kekulize_flag = True),    
    OperatorSmarts("Heck Reaction 2, Intramolecular", 
                   "[c+0:5][C+0:1]=[C+0:2]-@[C,c;+0:3].[F,Cl,Br,I;+0H:4]>>([*:5][*:1]=[*:2].[*:3][*:4])",
                   (1, 1),(1, ),
                   kekulize_flag = True),
    ...
    OperatorSmarts("Esterification with Alkyl and Aryl Halides", 
                   "[C+0:2](=[O+0:3])[O+0:4]!@[CX4,c;+0:5].[F,Cl,Br,I;+0H:6]>>[*:2](=[*:3])[*:4].[*:5][*:6]",
                   (1, 1),(1, 1)),
    OperatorSmarts("Esterification with Alkyl and Aryl Halides, Intramolecular", 
                   "[C+0:2](=[O+0:3])[O+0:4]@[CX4,c;+0:5].[F,Cl,Br,I;+0H:6]>>([*:2](=[*:3])[*:4].[*:5][*:6])",
                   (1, 1),(1, )),
    # Allylic, Benzylic Halogenation (Wohl–Ziegler Reaction)
    OperatorSmarts("Allylic, Benzylic Halogenation", 
                   "[C$(*=C),c;+0:1][CX4!$(*~[O,N,S])+0:2][F,Cl,Br,I;+0:3].[F,Cl,Br,I;+0H:4]>>[*:1][*:2].[*:3][*:4]",
                   (1, 1),(1, 1)),
    # Phosgene Production
    OperatorSmarts("Phosgene Production", 
    ...
    OperatorSmarts("Glycol Cleavage by Oxidation", 
                   "[C!$(*(=O)=O)+0:1]=[O+0:5].[C!$(*(=O)=O)+0:2]=[O+0:6].[O+0H2:3]>>[*:1]([*:5])[*:2][*:6].[*:3]=[O]",
                   (1, 1, 1),(1, 0.5),
                   kekulize_flag = True),
    OperatorSmarts("Glycol Cleavage by Oxidation Intramolecular", 
    ...
    OperatorSmarts("Hydrodeoxygenation of Alcohol, Classic Synthesis of Aldehydes from Carboxylic Acids", 
                   "[C,c;+0!H0:1].[O+0H2:2]>>[*:1][*:2].[H][H]",
                   (1, 1),(1, 1)),      
    # Dehydration of Alcohols
    OperatorSmarts("Dehydration of Alcohol", 
                   "[C+0:1]=[C+0:2].[O+0H2:3]>>[*:1][*:2][*:3]", 
                   (1, 1),(1, ),
                   kekulize_flag = True),  
    # Dehydration of Alcohols, 2-step
    OperatorSmarts("Dehydration of Alcohol, 2-step", 
                   "([C+0:1]=[C+0:2].[C+0:4]=[C+0:5]).[O+0H2:3]>>([*:1][*:2][*:3].[*:4][*:5][O])", 
                   (1, 2),(1, ),
                   kekulize_flag = True,
                   number_of_steps = 2),       
    # Selective Oxidation of Alcohols
    OperatorSmarts("Selective Oxidation of Alcohols", 
                   "[C+0:1]=[O+0:2].[O+0H2:3]>>[*:1][*:2].[*:3]=[O]",
                   (1, 1),(1, 0.5),
                   kekulize_flag = True),         
    # Diol Carboxylation
    OperatorSmarts("Diol Carboxylation", 
                   "[O+0H2:1].[C+0:2]1[C+0:3][O+0:4][C+0:6](=[O+0:7])[O+0:5]1>>[*:1][*:2][*:3][*:4].[*:5]=[*:6]=[*:7]",
                   (1, 1),(1, 1)),      
        # add n=1, doi.org/10.1021/cs500301d   
    OperatorSmarts("Diol Carboxylation 2", 
    ...
    OperatorSmarts("Hydrolysis of Ethers, Esters, Anhydrides", 
                   "[C,c;+0:1][O+0H:2].[C,c;+0:3][O+0H:4]>>[*:1][*:2][*:3].[*:4]",
                   (1, 1),(1, 1)),    
    OperatorSmarts("Hydrolysis of Ethers, Esters, Anhydrides, Intramolecular", 
                   "([C,c;+0:1][O+0H:2].[C,c;+0:3][O+0H:4])>>[*:1][*:2][*:3].[*:4]",
                   (1, ),(1, 1),
                   Retro_Not_Aromatic = True), 
    # Hydrogenolysis of Ethers
    OperatorSmarts("Hydrogenolysis of Ethers", 
                   "[C,c;+0!H0;!$(*=O):1].[O+0H:2][C,c;+0;!$(*=O):3]>>[*:1][*:2][*:3].[H][H]", 
                   (1, 1),(1, 1)),  
    OperatorSmarts("Hydrogenolysis of Ethers Intramolecular", 
                   "([C,c;+0!H0;!$(*=O):1].[O+0H:2][C,c;+0;!$(*=O):3])>>[*:1][*:2][*:3].[H][H]", 
                   (1, ),(1, 1),
                   Retro_Not_Aromatic = True),  
    # Williamson Ether Synthesis, Ullmann Condensation           doesn't work with tertiary alkyl halides or alkenyl or alkynyl halide     
    OperatorSmarts("Williamson Ether Synthesis, Ullmann Condensation", 
                   "[C,c;!$(*=O);+0:1][O+0:2]!@[CX4!H0,c;+0:3].[F,Cl,Br,I;+0H:4]>>[*:1][*:2].[*:3][*:4]",
                   (1, 1),(1, 1)),  
    OperatorSmarts("Williamson Ether Synthesis, Ullmann Condensation, Intramolecular", 
                   "[C,c;!$(*=O);+0:1][O+0:2]@[CX4!H0,c;+0:3].[F,Cl,Br,I;+0H:4]>>([*:1][*:2].[*:3][*:4])",
                   (1, 1),(1, )),      
    # Ether Cleavage                             Ar-O-R + HI -> Ar-OH + IR; Ar-O-Ar stable
    OperatorSmarts("Ether Cleavage", 
                   "[C,c;+0;!$(*=O):1][O+0H:2].[C;+0;!$(*=O):3][F,Cl,Br,I;+0:4]>>[*:1][*:2][*:3].[*:4]",
                   (1, 1),(1, 1)), 
    OperatorSmarts("Ether Cleavage, Intramolecular", 
                   "([C,c;+0;!$(*=O):1][O+0H:2].[C;+0;!$(*=O):3][F,Cl,Br,I;+0:4])>>[*:1][*:2][*:3].[*:4]",
                   (1, ),(1, 1),
                   Retro_Not_Aromatic = True),  
    # Ether Synthesis by Dehydration       tertiary alcohols dehydration to alkenes
    OperatorSmarts("Ether Synthesis by Dehydration", 
                   "[C,c;!$(*=O);+0:1][O+0:2]!@[C,c;!$(*=O);+0:3].[O+0H2:4]>>[*:1][*:2].[*:3][*:4]",
                   (1, 1),(1, 1)), 
    OperatorSmarts("Ether Synthesis by Dehydration, Intramolecular", 
                   "[C,c;!$(*=O);+0:1][O+0:2]@[C,c;!$(*=O);+0:3].[O+0H2:4]>>([*:1][*:2].[*:3][*:4])",
                   (1, 1),(1, ),
                   kekulize_flag = True), 
    # Epoxides Ring Opening            
    OperatorSmarts("Epoxides Ring Opening", 
    ...
    OperatorSmarts("Keto-enol Tautomerization", 
                   "[C,N;+0:1]=[C+0:2][O,N;+0!H0:3]>>[*:1][*:2]=[*:3]",  
                   (1, ),(1, ),
                   kekulize_flag = True),             # Tautomerism Cyclohexadienone
    OperatorSmarts("Keto-enol Tautomerization Reverse",                    
                   "[C,N;+0!H0:1][C+0:2]=[O,N;+0:3]>>[*:1]=[*:2][*:3]", 
                   (1, ),(1, ),
                   kekulize_flag = True),   
    # Decarbonylation of Aldehydes
    OperatorSmarts("Decarbonylation of Aldehydes", 
                   "[C,c;+0!H0:1].[C-]#[O+]>>[*:1][C]=[O]",
                   (1, 1),(1, )),        
    # Ketonization
    OperatorSmarts("Ketonization", 
                   "[*;!O;+0:1][C+0:2](=[O+0:3])-!@[*;!O;+0:5].[O+0:4]=[C+0:6]=[O+0:7].[O+0H2:8]>>[*:1][*:2](=[*:3])[*:4].[*:5][*:6](=[*:7])[*:8]",
                   (1, 1, 1),(1, 1)), 
    OperatorSmarts("Ketonization, Intramolecular", 
                   "[*;!O;+0:1][C+0:2](=[O+0:3])-@[*;!O;+0:5].[O+0:4]=[C+0:6]=[O+0:7].[O+0H2:8]>>([*:1][*:2](=[*:3])[*:4].[*:5][*:6](=[*:7])[*:8])",
                   (1, 1, 1),(1, ),
                   kekulize_flag = True),
    # Oxidative Esterification of Aldehydes and Alcohols   
    OperatorSmarts("Oxidative Esterification of Aldehydes and Alcohols", 
                   "[C+0;!$(*=O):5][O+0:1]!@[C+0:3]=[O+0:2].[O+0H2:4]>>[*:5][*:1].[*:2]=[*:3].[*:4]=[O]",
                   (1, 1),(1, 1, 0.5)),  
    OperatorSmarts("Oxidative Esterification of Aldehydes and Alcohols, Intramolecular", 
                   "[C+0;!$(*=O):5][O+0:1]@[C+0:3]=[O+0:2].[O+0H2:4]>>([*:5][*:1].[*:2]=[*:3]).[*:4]=[O]",
                   (1, 1),(1, 0.5),
                   kekulize_flag = True), 
    # McMurry Reaction
    OperatorSmarts("McMurry Reaction", 
                   "[*+0:1][C+0;!$(*-O):2]=!@[C+0;!$(*-O):6][*+0:5].[O+0H2:3].[O+0H2:7]>>[*:1][*:2](=[*:3]).[*:5][*:6](=[*:7]).[H][H]",
                   (1, 1, 1),(1, 1, 2),
                   Retro_Not_Aromatic = True), 
    OperatorSmarts("McMurry Reaction, Intramolecular", 
                   "[*+0:1][C+0;!$(*-O):2]=@[C+0;!$(*-O):6][*+0:5].[O+0H2:3].[O+0H2:7]>>([*:1][*:2](=[*:3]).[*:5][*:6](=[*:7])).[H][H]",
                   (1, 1, 1),(1, 2),
                   kekulize_flag = True,
                   Retro_Not_Aromatic = True),
    # Hydration of Ketone and Aldehyde
    OperatorSmarts("Hydration of Ketone and Aldehyde", 
                   "[C+0;!$(*-OC);!$(*(O)(O)(O));!$(*=O):2]([O+0H:3])[O+0H:4]>>[*:2]=[*:3].[*:4]",
                   (1, ),(1, 1),
                   Retro_Not_Aromatic = True),          
    # Hemiacetal Dissociation and Formation, Addition of Alcohols to Carbonyl Groups
    OperatorSmarts("Hemiacetal Dissociation, Addition of Alcohols to Carbonyl Groups Reverse", 
                   "[CX3;!$(*O):1]=[O+0:2].[O+0H:3][C;!$(*=O):4]>>[*:1]([*:2])[*:3][*:4]",
                   (1, 1),(1, )), 
    OperatorSmarts("Hemiacetal Dissociation, Addition of Alcohols to Carbonyl Groups Reverse, Intramolecular", 
                   "([C;!$(*O):1]=[O+0:2].[O+0H:3][C;!$(*=O):4])>>[*:1]([*:2])[*:3][*:4]",
                   (1, ),(1, )),     # could use Retro_Not_Aromatic = True but ngthermo doesn't support *C=C=O anyway
    OperatorSmarts("Hemiacetal Formation, Addition of Alcohols to Carbonyl Groups", 
                   "[C;!$(*=O):1]([O+0H:2])!@[O+0:3][C;!$(*=O):4]>>[*:1]=[*:2].[*:3][*:4]",
                   (1, ),(1, 1)),
    OperatorSmarts("Hemiacetal Formation, Addition of Alcohols to Carbonyl Groups, Intramolecular", 
                   "[C;!$(*=O):1]([O+0H:2])@[O+0:3][C;!$(*=O):4]>>([*:1]=[*:2].[*:3][*:4])",
                   (1, ),(1, )),      # could use kekulize_flag = True for furan but ngthermo doesn't support *C=C=O anyway
    # Reduction of Carbonyl Groups
    OperatorSmarts("Reduction of Carbonyl Groups", 
                   "[C,c,N;+0:1][C;H2,H3;+0:2].[O+0H2:3]>>[*:1][*:2]=[*:3].[H][H]",
                   (1, 1),(1, 2),
                   Retro_Not_Aromatic = True),    
    # a-Halogenation
    OperatorSmarts("a-Halogenation", 
    ...
    OperatorSmarts("Wittig Reaction (Combined with Ylide Formation)", 
                   "[C+0;!$(*~[O,S,N]):1]=!@[C+0X3;!$(*-O):4]>>[*:1][Br].[*:4]=[O]",
                   (1, ),(1, 1),
                   enthalpy_correction = -141.69,    # DFT by QE
                   Retro_Not_Aromatic = True),
    OperatorSmarts("Wittig Reaction, Intramolecular (Combined with Ylide Formation)", 
    ...
    OperatorSmarts("Synthesis of Enol Ethers from Aldehyde and Alcohol", 
    ...
    OperatorSmarts("Carboxylic Acids Decarboxylation", 
                   "[C,c;+0!H0:1].[O+0:3]=[C+0:2]=[O+0:4]>>[*:1][*:2](=[*:3])[*:4]",
                   (1, 1),(1, )),     
    # Acid Anhydrides React with Alcohols to Form Esters
    OperatorSmarts("Acid Anhydrides React with Alcohols to Form Esters", 
                   "[C+0;!$(*[OH]):1](=[O+0:2])[O+0!R:3][C,c;+0;!$(*=O):6].[C+0;!$(*([OH])[OH]):4](=[O+0:5])[O+0H:7]>>[*:1](=[*:2])[*:3][C*:4](=[*:5]).[*:6][*:7]",
                   (1, 1),(1, 1)), 
    OperatorSmarts("Acid Anhydrides React with Alcohols to Form Esters, Intramolecular", 
                   "([C+0;!$(*[OH]):1](=[O+0:2])[O+0!R:3][C;+0;!$(*=O):6].[C+0;!$(*([OH])[OH]):4](=[O+0:5])[O+0H:7])>>[*:1](=[*:2])[*:3][C*:4](=[*:5]).[*:6][*:7]",
                   (1, ),(1, 1),
                   ring_issue = True),       # removed aromatic c:6 to avoid issue for things like O=C(Br)Oc1coc(C(=O)O)c1
    # Oxidation of Butane
    OperatorSmarts("Oxidation of Butane", 
    ...
    # Enols and Enolates ##################################################
    # Aldol Condensation   
    OperatorSmarts("Aldol Condensation", 
                   "[C!$(*[OH])+0:2](=[O+0:3])[C+0:4]-!@[C!$(*([OH])[OH])+0X4:6]([O+0H:5])>>[*:2](=[*:3])[*:4].[*:5]=[*:6]",
                   (1, ),(1, 1),
                   Retro_Not_Aromatic = True),  
    OperatorSmarts("Aldol Condensation, Intramolecular", 
                   "[C!$(*[OH])+0:2](=[O+0:3])[C+0:4]-@[C!$(*([OH])[OH])+0X4:6]([O+0H:5])>>([*:2](=[*:3])[*:4].[*:5]=[*:6])",
                   (1, ),(1, ),
                   Retro_Not_Aromatic = True), 
    OperatorSmarts("Aldol Condensation (2H)", 
                   "[C!$(*[OH])+0:2](=[O+0:3])[C+0:4]=!@[C!$(*[OH])+0X3:6].[O+0H2:5]>>[*:2](=[*:3])[*:4].[*:5]=[*:6]",
                   (1, 1),(1, 1),
                   Retro_Not_Aromatic = True),
    OperatorSmarts("Aldol Condensation (2H), Intramolecular", 
                   "[C!$(*[OH])+0:2](=[O+0:3])[C+0:4]=@[C!$(*[OH])+0X3:6].[O+0H2:5]>>([*:2](=[*:3])[*:4].[*:5]=[*:6])",
                   (1, 1),(1, ),
                   kekulize_flag = True,
                   Retro_Not_Aromatic = True),     
    # Claisen Condensation (including crossed)
    OperatorSmarts("Claisen Condensation", 
    ...
    OperatorSmarts("Dieckmann Condensation",                    
                   "[C!$(*[OH])+0:2](=[O+0:3])-@[C+0:8][C!$(*[OH])+0:6]=[O+0:5].[O+0H:7][C+0:4]>>([*:2](=[*:3])[*:7][*:4].[*:5]=[*:6][*:8])",
                   (1, 1),(1, ),
                   kekulize_flag = True), 
    # Michael Reaction
    OperatorSmarts("Michael Reaction", 
                   "[C+0:1](=[O+0:2])[C+0:3](-!@[C+0:6][C+0!H0:7][C+0:8]=[O+0:9])[C+0:4]=[O+0:5]>>[*:1](=[*:2])[*:3][*:4]=[*:5].[*:6]=[*:7][C+0:8]=[O+0:9]",
                   (1, ),(1, 1),
                   Retro_Not_Aromatic = True),
    OperatorSmarts("Michael Reaction, Intramolecular", 
                   "[C+0:1](=[O+0:2])[C+0:3](-@[C+0:6][C+0!H0:7][C+0:8]=[O+0:9])[C+0:4]=[O+0:5]>>([*:1](=[*:2])[*:3][*:4]=[*:5].[*:6]=[*:7][C+0:8]=[O+0:9])",
                   (1, ),(1, ),
                   Retro_Not_Aromatic = True),
        # with a 6-membered ring
    OperatorSmarts("Michael Reaction with Cyclic Ketones", 
                   "[C+0:1]1(=[O+0:2])[C+0:3]([C+0:10][C+0:11][C+0:12][C+0:13]1)-!@[C+0:6][C+0!H0:7][C+0:8]=[O+0:9]>>[*:1]1(=[*:2])[*:3][*:10][*:11][*:12][*:13]1.[*:6]=[*:7][*:8]=[*:9]",
                   (1, ),(1, 1),
                   Retro_Not_Aromatic = True),
    OperatorSmarts("Michael Reaction with Cyclic Ketones, Intramolecular",   
                   "[C+0:1]1(=[O+0:2])[C+0:3]([C+0:10][C+0:11][C+0:12][C+0:13]1)-@[C+0:6][C+0!H0:7][C+0:8]=[O+0:9]>>([*:1]1(=[*:2])[*:3][*:10][*:11][*:12][*:13]1.[*:6]=[*:7][*:8]=[*:9])",
                   (1, ),(1, ),
                   Retro_Not_Aromatic = True),
    # Robinson Annulation (Michael-aldol sequence)
    OperatorSmarts("Robinson Annulation", 
    ...
    OperatorSmarts("Esterification, Acid Anhydride Formation", 
                   "[C+0:2](=[O+0:3])!@[O+0:6][C,c;+0:5].[O+0H2:4]>>[*:2](=[*:3])[*:4].[*:5][*:6]",
                   (1, 1),(1, 1)),    
    OperatorSmarts("Esterification, Acid Anhydride Formation, Intramolecular", 
                   "[C+0:2](=[O+0:3])@[O+0:6][C,c;+0:5].[O+0H2:4]>>([*:2](=[*:3])[*:4].[*:5][*:6])",
                   (1, 1),(1, ),
                   kekulize_flag = True),     
    # Esterification of Acid Halides
    OperatorSmarts("Esterification of Acid Halides", 
                   "[C+0:1](=[O+0:2])!@[O+0:5][C,c;!$(*=O);+0:4].[F,Cl,Br,I;+0H:3]>>[*:1](=[*:2])[*:3].[*:4][*:5]",
                   (1, 1),(1, 1)),
    OperatorSmarts("Esterification of Acid Halides, Intramolecular", 
                   "[C+0:1](=[O+0:2])@[O+0:5][C,c;!$(*=O);+0:4].[F,Cl,Br,I;+0H:3]>>([*:1](=[*:2])[*:3].[*:4][*:5])",
                   (1, 1),(1, ),
                   kekulize_flag = True),             
    # Ester Reduction
        # To Aldehydes
    OperatorSmarts("Ester Reduction to Aldehydes and Alcohols", 
                   "[O+0H:1].[C+0!H0:2]=[O+0:3]>>[*:1][*:2]=[*:3].[H][H]",
                   (1, 1),(1, 1)),
    OperatorSmarts("Ester Reduction to Aldehydes and Alcohols, Intramolecular", 
                   "([O+0H:1].[C+0!H0:2]=[O+0:3])>>[*:1][*:2]=[*:3].[H][H]",
                   (1, ),(1, 1),
                   ring_issue = True,
                   Retro_Not_Aromatic = True), 
        # To Alcohols
    OperatorSmarts("Ester Reduction to Alcohols", 
                   "[O+0H:1].[C+0;H2,H3:2][O+0H:3]>>[*:1][*:2]=[*:3].[H][H]",
                   (1, 1),(1, 2)), 
    OperatorSmarts("Ester Reduction to Alcohols, Intramolecular", 
                   "([*+0:4][O+0H:1].[C+0;H2,H3:2][O+0H:3])>>[*:4][*:1][*:2]=[*:3].[H][H]",
                   (1, ),(1, 2),
                   Retro_Not_Aromatic = True),
    # Transesterification
    OperatorSmarts("Transesterification", 
                   "[C+0:1](=[O+0:2])!@[O+0:3][*+0:4].[*+0:6][O+0H;!$(*-C=O):5]>>[*:1](=[*:2])[*:5][*:6].[*:4][*:3]",
                   (1, 1),(1, 1)), 
    OperatorSmarts("Transesterification, Intramolecular",   
                   "[C+0:1](=[O+0:2])@[O+0:3][*+0:4].[*+0:6][O+0H;!$(*-C=O):5]>>([*:1](=[*:2])[*:5][*:6].[*:4][*:3])",
                   (1, 1),(1, ),
                   kekulize_flag = True),
    # Ester Hydrogenation                   
    ...
    OperatorSmarts("Naphthalene Oxidation with CrO3", 
                   ...
                   (1, 1),(1, 1.5)), 
        # with V2O5 catalyst
    OperatorSmarts("Naphthalene Oxidation with V2O5 Catalyst", 
    ...
    # Oxidation of Aromatic Alkanes
        # with -CH3
    OperatorSmarts("Oxidation of Aromatic Alkanes Ar-CH3", 
                   "[c+0:1][C+0:2](=[O+0:3])[O+0H].[O+0H2:4]>>[*:1][*:2].[*:3]=[*:4]",
                   (1, 1),(1, 1.5)), 
        # with -CH2CH3
    OperatorSmarts("Oxidation of Aromatic Alkanes Ar-CH2CH3", 
                   "[c+0:1][C+0:2](=[O+0:4])[O+0H].[O+0]=[C+0:3]=[O+0].[O+0H2:5]>>[*:1][*:2][*:3].[*:4]=[*:5]",
                   (1, 1, 2),(1, 2.5)), 
        # with -(CH3)CH3
    OperatorSmarts("Oxidation of Aromatic Alkanes Ar-(CH3)CH3", 
                   "[c+0:1][C+0:2](=[O+0:4])[O+0H].[O+0]=[C+0:3]=[O+0].[O+0H2:5]>>[*:1][*:2]([*:3])[C].[*:4]=[*:5]",
                   (1, 2, 3),(1, 4.5)),  
        # with long chain
    OperatorSmarts("Oxidation of Aromatic Alkanes Ar-long_chain", 
                   "[c+0:1][C+0:2](=[O+0:5])[O+0H].[C+0!H3:4][C+0:3](=[O+0])[O+0H].[O+0H2:6]>>[*:1][*:2][*:3][*:4].[*:5]=[*:6]",
                   (1, 1, 1),(1, 2.5)),  
        # Ring-opening
    OperatorSmarts("Oxidation of Aromatic Alkanes Ring-opening 1", 
                   "([c+0:1][C+0:2](=[O+0:4])[O+0H].[C+0:3](=[O+0])[O+0H]).[O+0H2:5]>>[*:1][*:2][*:3].[*:4]=[*:5]",
                   (1, 1),(1, 2.5)), 
    OperatorSmarts("Oxidation of Aromatic Alkanes Ring-opening 2", 
    ...
    # Oxidation of Aromatic Alkanes, 2-step
        # with -CH3
    OperatorSmarts("Oxidation of Aromatic Alkanes Ar-CH3, 2-step", 
                   "([c+0:1][C+0:2](=[O+0:3])[O+0H].[c+0:5][C+0:6](=[O+0])[O+0H]).[O+0H2:4]>>([*:1][*:2].[*:5][*:6]).[*:3]=[*:4]",
                   (1, 2),(1, 3),
                   number_of_steps = 2),
    # Oxidation of Aldehyde & Aromatic Alkanes
        # with -CH3
    OperatorSmarts("Oxidation of Aldehyde & Aromatic Alkanes, 2-step", 
                   "([C+0:1](=[O+0:2])[O+0H:3].[c+0:5][C+0:6](=[O+0])[O+0H]).[O+0H2:4]>>([*:1]=[*:2].[*:5][*:6]).[*:3]=[*:4]",
                   (1, 1),(1, 2),
                   number_of_steps = 2), 
    # Oxidation of Alcohol & Aromatic Alkanes
        # with -CH3
    OperatorSmarts("Oxidation of Alcohol & Aromatic Alkanes, 2-step", 
                   "([C+0:1](=[O+0:2])[O+0H:3].[c+0:5][C+0:6](=[O+0])[O+0H]).[O+0H2:4]>>([*:1][*:2].[*:5][*:6]).[*:3]=[*:4]",
                   (1, 2),(1, 2.5),
                   number_of_steps = 2),                        
    # Halogenation of Aromatic Alkanes
    OperatorSmarts("Halogenation of Aromatic Alkanes", 
                   "[c+0:1][C+0:2][F,Cl,Br,I;+0:3].[F,Cl,Br,I;+0H:4]>>[*:1][*:2].[*:3][*:4]",
                   (1, 1),(1, 1)), 
    # Electrophilic Aromatic Alkylation with Alkenes
    OperatorSmarts("Electrophilic Aromatic Alkylation with Alkenes", 
                   "[c+0:1]-!@[C+0:2][C+0!H0:3]>>[*:1].[*:2]=[*:3]",
                   (1, ),(1, 1),
                   Retro_Not_Aromatic = True), 
    OperatorSmarts("Electrophilic Aromatic Alkylation with Alkenes, Intramolecular", 
                   "[c+0:1]-@[C+0:2][C+0!H0:3]>>([*:1].[*:2]=[*:3])",
                   (1, ),(1, ),
                   Retro_Not_Aromatic = True),              
    # Electrophilic Aromatic Alkylation with Alcohols
    OperatorSmarts("Electrophilic Aromatic Alkylation with Alcohols", 
                   "[c+0:1]-!@[C+0;!$(*=O):2].[O+0H2:3]>>[*:1].[*:2][*:3]",
                   (1, 1),(1, 1)), 
    OperatorSmarts("Electrophilic Aromatic Alkylation with Alcohols, Intramolecular", 
                   "[c+0:1]-@[C+0;!$(*=O):2].[O+0H2:3]>>([*:1].[*:2][*:3])",
                   (1, 1),(1, )), # may use kekulize_flag = True but need to rewrite it first   
    # Furan Carboxylation
    OperatorSmarts("Furan Carboxylation", 
                   "[o+0:1]1:[c+0:2]([C+0:9](=[O+0:8])[O+0H:10]):[c+0:3]:[c+0:4]:[c+0:5]:1>>[*:1]1:[*:2]:[*:3]:[*:4]:[*:5]:1.[*:8]=[*:9]=[*:10]",
                   (1, ),(1, 1)),
    # Furan Carboxylation, 2-step
    OperatorSmarts("Furan Carboxylation, 2-step", 
    ...
    OperatorSmarts("Friedel-Crafts Hydroxyalkylation", 
                   "[c+0:1]-!@[CX4!$(*(-O)-O)!$(*(-O)(-O)-O)+0:2][O+0H:3]>>[*:1].[*:2]=[*:3]",
                   (1, ),(1, 1),
                   Retro_Not_Aromatic = True),
    OperatorSmarts("Friedel-Crafts Hydroxyalkylation, Intramolecular", 
                   "[c+0:1]-@[CX4!$(*(-O)-O)!$(*(-O)(-O)-O)+0:2][O+0H:3]>>([*:1].[*:2]=[*:3])",
                   (1, ),(1, ),
                   Retro_Not_Aromatic = True),
    OperatorSmarts("Friedel-Crafts Hydroxyalkylation(BPA Synthesis)", 
                   "[c+0:1]-!@[CX4!$(*-O)!$(*(-O)-O)+0:3]-!@[c+0:2].[O+0H2:4]>>[*:1].[*:2].[*:3]=[*:4]",
                   (1, 1),(1, 1, 1),
                   Retro_Not_Aromatic = True),
    # Hock Process
    OperatorSmarts("Hock Process", 
    ...
    OperatorSmarts("Dehydrogenation of Ethylbenzene", 
    ...
    OperatorSmarts("Tennessee Eastman Acetic Anhydride Process", 
                   "[C:1](=[O:2])[O:3][C:4](=[O:5])[C:6]>>[C+0:1](=[O+0:2])[O+0:3][C+0:6].[*-1:4]#[*+1:5]",
                   (1, ),(1, 1),
                   Retro_Not_Aromatic = True), 
    # Reppe Process
    OperatorSmarts("Reppe Process", 
    ...
    OperatorSmarts("Oxidation of Pyranosides", 
...
#                   (1, 1),(1, 0.5)),
    ...
    OperatorSmarts("Dehydration of Amides", 
                   "[C+0:1]#[N+0:3].[O+0H2:2]>>[*:1](=[*:2])[*:3]",
                   (1, 1),(1, )),    
    # ketone reductive amination  
    OperatorSmarts("Ketone Reductive Amination", 
                   "[CX4!$(*[O,S])!$(*(N)N)!$(*(N)(N)N)!H0+0:2]-!@[N+0X3:6].[O+0H2:3]>>[*:2]=[*:3].[*:6].[H][H]",
                   (1, 1),(1, 1, 1),
                   Retro_Not_Aromatic = True),     
    OperatorSmarts("Ketone Reductive Amination, Intramolecular", 
                   "[CX4!$(*[O,S])!$(*(N)N)!$(*(N)(N)N)!H0+0:2]-@[N+0X3:6].[O+0H2:3]>>([*:2]=[*:3].[*:6]).[H][H]",
                   (1, 1),(1, 1),
                   Retro_Not_Aromatic = True),         
    # Alkylation or Acylation of Amines
    OperatorSmarts("Alkylation or Acylation of Amines", 
                   "[C,c;+0:1]-!@[NX3,n;+0:3].[F,Cl,Br,I;+0H:2]>>[*:1][*:2].[*:3]",
                   (1, 1),(1, 1)),  
    OperatorSmarts("Alkylation or Acylation of Amines, Intramolecular", 
                   "[C+0:1]-@[NX3+0:3].[F,Cl,Br,I;+0H:2]>>([*:1][*:2].[*:3])",
                   (1, 1),(1, ),
                   kekulize_flag = True),     
    
    # Alkylation of Tertiary Amines                             # generates ions, careful
    OperatorSmarts("Alkylation of Tertiary Amines", 
    ...
    OperatorSmarts("Amine Alkylation with Alcohols or Primary Amines", 
                   "[C,c;!$(*=O)+0:1]-!@[NX3,n;+0:3].[OH2,NH3;+0:2]>>[*:1][*:2].[*:3]",
                   (1, 1),(1, 1)),
    OperatorSmarts("Amine Alkylation with Alcohols or Primary Amines, Intramolecular", 
                   "[C!$(*=O)+0:1]-@[NX3+0:3].[OH2,NH3;+0:2]>>([*:1][*:2].[*:3])",
                   (1, 1),(1, ),
                   kekulize_flag = True),   
    # Synthesis of Amides with Carboxylic Acid
    OperatorSmarts("Synthesis of Amides with Carboxylic Acid", 
                   "[CX3!$(*[OH,SH])+0:2](=[O+0:3])-!@[N+0X3:5].[O+0H2:4]>>[*:2](=[*:3])[*:4].[*:5]",
                   (1, 1),(1, 1)),      
    OperatorSmarts("Synthesis of Amides with Carboxylic Acid, Intramolecular", 
                   "[CX3!$(*[OH,SH])+0:2](=[O+0:3])-@[N+0X3:5].[O+0H2:4]>>([*:2](=[*:3])[*:4].[*:5])",
                   (1, 1),(1, ),
                   kekulize_flag = True),     
    # Synthesis of Amides with Acid Anhydrides or Esters
    OperatorSmarts("Synthesis of Amides with Acid Anhydrides or Esters", 
                   "[C+0:2](=[O+0:3])-!@[N+0X3:5].[O+0H:4][C,c;+0:6]>>[*:2](=[*:3])[*:4][*:6].[*:5]",
                   (1, 1),(1, 1)),
    OperatorSmarts("Synthesis of Amides with Acid Anhydrides or Esters, Intramolecular", 
                   "[C+0:2](=[O+0:3])-@[N+0X3:5].[O+0H:4][C,c;+0:6]>>([*:2](=[*:3])[*:4][*:6].[*:5])",
                   (1, 1),(1, ),
                   kekulize_flag = True),    
    # Hydrolysis of Amides
    OperatorSmarts("Hydrolysis of Amides", 
                   "[CX3+0:2](=[O+0:3])[O+0H:4].[N+0X3!H0:5]>>[*:2](=[*:3])[*:5].[*:4]",
                   (1, 1),(1, 1)),     
    OperatorSmarts("Hydrolysis of Amides, Intramolecular", 
                   "([CX3+0:2](=[O+0:3])[O+0H:4].[N+0X3!H0:5])>>[*:2](=[*:3])[*:5].[*:4]",
                   (1, ),(1, 1),
                   ring_issue = True,
                   Retro_Not_Aromatic = True),             
    # Hofmann Elimination
    OperatorSmarts("Hofmann Elimination R-NH2", # can not garantee N has 3 identical side chains, so limit to N(CH3)(CH3)CH3
                   "[C+0:1]=[C+0:2].[N+0:3]([C+0H3:4])([C+0H3])[C+0H3].[F,Cl,Br,I;+0H:5]>>[*:1][*:2][*:3].[*:4][*:5]",
                   (1, 1, 3),(1, 3),
                   kekulize_flag = True),  
    OperatorSmarts("Hofmann Elimination R-NHR", # similar problem as above, so limit to N(CH3)(CH3)R
                   "[C+0:1]=[C+0:2].[N+0:3]([C+0:4])([C+0H3])[C+0H3].[F,Cl,Br,I;+0H:5]>>[*:1][*:2][*:3][*:4].[C][*:5]",
                   (1, 1, 2),(1, 2),
                   kekulize_flag = True),  
    OperatorSmarts("Hofmann Elimination R-NHR Ringopening", # similar problem as above
                   "([C+0:1]=[C+0:2].[N+0:3]([C+0:4])([C+0H3])[C+0H3]).[F,Cl,Br,I;+0H:5]>>[*:1][*:2][*:3][*:4].[C][*:5]",
                   (1, 2),(1, 2),
                   kekulize_flag = True),  
    OperatorSmarts("Hofmann Elimination R-NRR", 
                   "[C+0:1]=[C+0:2].[N+0:3]([C+0:4])([C+0:6])!@[C+0:7].[F,Cl,Br,I;+0H:5]>>[*:1][*:2][*:3]([*:4])[*:6].[*:7][*:5]",
                   (1, 1, 1),(1, 1),
                   kekulize_flag = True),  
    OperatorSmarts("Hofmann Elimination R-NRR Ringopening", 
                   "([C+0:1]=[C+0:2].[N+0:3]([C+0:4])([C+0:6])-!@[C+0:7]).[F,Cl,Br,I;+0H:5]>>[*:1][*:2][*:3]([*:6])[*:4].[*:7][*:5]",
                   (1, 1),(1, 1),
                   kekulize_flag = True,
                   ring_issue = True),     
    # Hofmann Rearrangement
    OperatorSmarts("Hofmann Rearrangement", 
                   "[C,c;+0:1][N+0H2:5].[O+0:3]=[C+0:2]=[O+0:4].[F,Cl,Br,I;+0H:6]>>[*:1][*:2](=[*:3])[*:5].[*:6][*:6].[*:4]",
                   (1, 1, 2),(1, 1, 1)), 
    OperatorSmarts("Hofmann Rearrangement with Alcohols", 
                   "[C,c;+0:1][N+0H:5][C+0:2](=[O+0:3])-!@[O+0:4][C+0!$(*=[O,S]):7].[F,Cl,Br,I;+0H:6]>>[*:1][*:2](=[*:3])[*:5].[*:6][*:6].[*:7][*:4]",
                   (1, 2),(1, 1, 1)),                 
    # Hydroamination of Alkenes
    OperatorSmarts("Hydroamination of Alkenes", 
                   "[C+0!H0:1][C+0:2]-!@[N+0X3:3]>>[*:1]=[*:2].[*:3]",
                   (1, ),(1, 1),
                   Retro_Not_Aromatic = True), 
    OperatorSmarts("Hydroamination of Alkenes, Intramolecular", 
                   "[C+0!H0:1][C+0:2]-@[N+0X3:3]>>([*:1]=[*:2].[*:3])",
                   (1, ),(1, ),
                   Retro_Not_Aromatic = True),  
    # Hydroamination of Alkynes
    OperatorSmarts("Hydroamination of Alkynes", 
                   "[C+0!H0:1]=[C+0:2]-!@[N+0X3:3]>>[*:1]#[*:2].[*:3]",
                   (1, ),(1, 1),
                   kekulize_flag = True), 
    OperatorSmarts("Hydroamination of Alkynes, Intramolecular", 
                   "[C+0!H0:1]=[C+0:2]-@[N+0X3:3]>>([*:1]#[*:2].[*:3])",
                   (1, ),(1, ),
                   kekulize_flag = True), 
    # Hydroamination of Dienes
    OperatorSmarts("Hydroamination of Dienes", 
                   "[C+0!H0:1][C+0:2]=[C+0:3][C+0:4]-!@[N+0X3:5]>>[*:1]=[*:2][*:3]=[*:4].[*:5]",
                   (1, ),(1, 1),
                   Retro_Not_Aromatic = True),
    OperatorSmarts("Hydroamination of Dienes, Intramolecular", 
                   "[C+0!H0:1][C+0:2]=[C+0:3][C+0:4]-@[N+0X3:5]>>([*:1]=[*:2][*:3]=[*:4].[*:5])",
                   (1, ),(1, ),
                   Retro_Not_Aromatic = True),        
    # Ring Opening of Epoxides by Amines
    OperatorSmarts("Ring Opening of Epoxides by Amines", 
                   "[N+0X3:4]-!@[C+0:1][C+0:2][O+0H:3]>>[*:1]1[*:2][*:3]1.[*:4]",
                   (1, ),(1, 1)),    
    # Aromatic Nitrosation
    OperatorSmarts("Aromatic Nitrosation", 
    ...
    OperatorSmarts("Secondary Amines with Nitrous Acid", 
    ...
    OperatorSmarts("Primary Amines with Nitrous Acid to Alcohols", 
                   "[C,c;+0:1][O+0H:3].[O+0H2:5].[N+0:2]#[N+0:4]>>[*:1][*:2].[*:3][*:4]=[*:5]",
                   (1, 1, 1),(1, 1)),    
    # Primary Amines with Nitrous Acid to Halides       
    OperatorSmarts("Primary Amines with Nitrous Acid to Halides", 
    ...
    OperatorSmarts("Primary Amines with Nitrous Acid to Alkenes", 
                   "[C+0:6]=[C+0:1].[O+0H2:5].[N+0:2]#[N+0:4]>>[*:6][*:1][*:2].[O][*:4]=[*:5]",
                   (1, 2, 1),(1, 1),
                   kekulize_flag = True),    
    # Tiffeneau–Demjanov Rearrangement       
    OperatorSmarts("Tiffeneau–Demjanov Rearrangement", 
                   "[O+0:1]=[C+0:2]([C+0:3])[C+0:5][C+0:4].[O+0H2:8].[N+0:6]#[N+0:7]>>[*:1][*:2]([*:3])([*:4])[*:5][*:6].[O][*:7]=[*:8]",
                   (1, 2, 1),(1, 1)),      
    # Sandmeyer Cyanation
    OperatorSmarts("Sandmeyer Cyanation", 
                   "[c+0:1][C+0:3]#[N+0:4].[O+0H2:6].[N+0:2]#[N+0:5]>>[*:1][*:2].[*:3]#[*:4].[O][*:5]=[*:6]",
                   (1, 2, 1),(1, 1, 1)),       
    # Reduction of Primary Aromatic Amines 
    OperatorSmarts("Reduction of Primary Aromatic Amines", 
                   "[c+0H:1].[O+0H2:4].[N+0:2]#[N+0:3]>>[*:1][*:2].[H][H].[O][*:3]=[*:4]",
                   (1, 2, 1),(1, 1, 1)),    
    # Cope Elimination 
    OperatorSmarts("Cope Elimination", 
                   "[C+0:1]=[C+0:2].[O+0H:6][N+0:3]([C+0:4])[C+0:5]>>[*:1][*:2][*:3]([*:4])[*:5].[O]=[*:6]",
                   (1, 1),(1, 0.5),
                   kekulize_flag = True),    
    OperatorSmarts("Cope Elimination, Intramolecular", 
                   "([C+0:1]=[C+0:2].[O+0H:6][N+0:3]([C+0:4])[C+0:5])>>[*:1][*:2][*:3]([*:4])[*:5].[O]=[*:6]",
                   (1, ),(1, 0.5),
                   kekulize_flag = True),    
    # Hemiaminal Formation
    OperatorSmarts("Hemiaminal Formation", 
                   "[CX4!$(*(O)O)+0:1]([O+0H:2])-!@[N,n;+0X3:3]>>[*:1]=[*:2].[*:3]",
                   (1, ),(1, 1),
                   kekulize_flag = True),            # need to kekulize or could have kekulize error, don't know why
    OperatorSmarts("Hemiaminal Formation, Reverse", 
                   "[CX3!$(*O)+0:1]=[O+0:2].[N,n;+0X3!H0:3]>>[*:1]([*:2])[*:3]",
                   (1, 1),(1, )),       
    # Hemiaminal Dehydration
    OperatorSmarts("Hemiaminal Dehydration", 
                   "[C+0:1]=[N+0:3].[O+0H2:2]>>[*:1]([*:2])[*:3]",
                   (1, 1),(1, ),
                   kekulize_flag = True),
    OperatorSmarts("Hemiaminal Dehydration, Reverse", 
    ...
    OperatorSmarts("Reduction of Nitroso", 
    ...
    OperatorSmarts("Imines from Aldehydes and Ketones", 
                   "[CX3!$(*[OH])+0:1]=!@[N+0:3].[O+0H2:2]>>[*:1]=[*:2].[*:3]",
                   (1, 1),(1, 1)),
    OperatorSmarts("Imines from Aldehydes and Ketones, Intramolecular", 
                   "[CX3!$(*[OH])+0:1]=@[N+0:3].[O+0H2:2]>>([*:1]=[*:2].[*:3])",
                   (1, 1),(1, ),
                   kekulize_flag = True),   
    OperatorSmarts("Imines from Aldehydes and Ketones Reverse", 
                   "[CX3!$(*[OH])+0:1]=[O+0:2].[N+0X3;H2,H3:3]>>[*:1]=[*:3].[*:2]",
                   (1, 1),(1, 1)),
    OperatorSmarts("Imines from Aldehydes and Ketones, Intramolecular Reverse", 
                   "([CX3!$(*[OH])+0:1]=[O+0:2].[N+0X3;H2,H3:3])>>[*:1]=[*:3].[*:2]",
                   (1, ),(1, 1),
                   ring_issue = True,
                   Retro_Not_Aromatic = True),             
    # Treatment of Aldehydes and Ketones with a Secondary Amine
    OperatorSmarts("Treatment of Aldehydes and Ketones with a Secondary Amine", 
                   "[C,N;+0:4]=[CX3!$(*[OH])+0:1]-!@[NX3+0:3].[O+0H2:2]>>[*:4][*:1]=[*:2].[*:3]",
                   (1, 1),(1, 1),
                   kekulize_flag = True),
    OperatorSmarts("Treatment of Aldehydes and Ketones with a Secondary Amine, Intramolecular", 
                   "[C,N;+0:4]=[CX3!$(*[OH])+0:1]-@[NX3+0:3].[O+0H2:2]>>([*:4][*:1]=[*:2].[*:3])",
                   (1, 1),(1, ),
                   kekulize_flag = True),
    OperatorSmarts("Treatment of Aldehydes and Ketones with a Secondary Amine Reverse", 
                   "[C,N;+0!H0:4][CX3!$(*[OH])+0:1]=[O+0:2].[N+0X3H:3]>>[*:4]=[*:1][*:3].[*:2]",
                   (1, 1),(1, 1),
                   Retro_Not_Aromatic = True),
    OperatorSmarts("Treatment of Aldehydes and Ketones with a Secondary Amine, Intramolecular Reverse", 
                   "([C,N;+0!H0:4][CX3!$(*[OH])+0:1]=[O+0:2].[N+0X3H:3])>>[*:4]=[*:1][*:3].[*:2]",
                   (1, ),(1, 1),
                   ring_issue = True,
                   Retro_Not_Aromatic = True),        
    # Oxime-Nitroso Tautomerization        
    OperatorSmarts("Oxime-Nitroso Tautomerization", 
                   "[C+0!H0:1][N+0:2]=[O+0:3]>>[*:1]=[*:2][*:3]", 
                   (1, ),(1, ),
                   Retro_Not_Aromatic = True),
    OperatorSmarts("Oxime-Nitroso Tautomerization Reverse", 
                   "[C+0:1]=[N+0:2][O+0H:3]>>[*:1][*:2]=[*:3]", 
                   (1, ),(1, ),
                   kekulize_flag = True),   
    # Reduction of Imines
    OperatorSmarts("Reduction of Imines", 
                   "[C+0!H0:1][N+0!H0:2]>>[*:1]=[*:2].[H][H]",
                   (1, ),(1, 1),
                   Retro_Not_Aromatic = True),   
    # Beckmann Rearrangement
    OperatorSmarts("Beckmann Rearrangement with Ketoximes", 
                   "[C,c;+0:1][C+0:2](=[O+0:4])[N+0H:3][C,c;+0:5]>>[*:1][*:2](=[*:3][*:4])[*:5]",
                   (1, ),(1, ),
                   kekulize_flag = True,
                   Retro_Not_Aromatic = True),    
    OperatorSmarts("Beckmann Rearrangement with Aldoximes", 
                   "[C,c;+0:1][C+0:2]#[N+0:3].[O+0H2:4]>>[*:1][*:2]=[*:3][*:4]",
                   (1, 1),(1, )),   
    # Beckmann Rearrangement from Ketones and Aldehydes
    OperatorSmarts("Beckmann Rearrangement from Ketones", 
                   "[C,c;+0:1][C+0:2](=[O+0:3])[N+0H:5][C,c;+0:4].[O+0H2:6]>>[*:1][*:2](=[*:3])[*:4].[*:5][*:6]",
                   (1, 1),(1, 1),
                   kekulize_flag = True,
                   Retro_Not_Aromatic = True),  
    OperatorSmarts("Beckmann Rearrangement from Aldehydes", 
    ...
    OperatorSmarts("Azide-Alkyne, Azide-Nitrile Cycloaddition", 
                   "[N+0:1]1[N+0]=[N+0][C,N;+0:4]=[C+0:5]1>>[*:1]=[N+1]=[N-1].[*:4]#[*:5]",
                   (1, ),(1, 1),
                   ring_issue = True,
                   kekulize_flag = True),    
    # Azide Salts as Nucleophiles
    OperatorSmarts("Azide Salts as Nucleophiles",    # works with carboxylic acid
                   "[C+0:1][N+0:3]=[N+1:4]=[N-1:5].[FH,ClH,BrH,IH,OH2;+0:2]>>[*:1][*:2].[*:3]=[*:4]=[*:5]",
                   (1, 1),(1, 1)),    
    # Reduction of Azides
    OperatorSmarts("Reduction of Azides", 
                   "[NX3+0H2:1].[N]#[N]>>[*:1]=[N+1]=[N-1].[H][H]",
                   (1, 1),(1, 1)),        
    # Curtius Rearrangement
    OperatorSmarts("Curtius Rearrangement", 
    ...
    OperatorSmarts("Esters from Diazoes and Carboxylic Acids", 
    ...
    OperatorSmarts("Hydrosulfide Anion Substitution with Alkyl Halides", 
                   "[C,c;+0:1]-!@[SX2+0:3].[F,Cl,Br,I;+0H:2]>>[*:1][*:2].[*:3]",
                   (1, 1),(1, 1)),      
    OperatorSmarts("Hydrosulfide Anion Substitution with Alkyl Halides, Intramolecular", 
                   "[C,c;+0:1]-@[SX2+0:3].[F,Cl,Br,I;+0H:2]>>([*:1][*:2].[*:3])",
                   (1, 1),(1, ),
                   kekulize_flag = True),      
    # Preparation of Sulfides
    OperatorSmarts("Preparation of Sulfides", 
                   "[C,c;+0:1]-!@[SX2+0:5][C,c;+0:3].[F,Cl,Br,I;+0H:2]>>[*:1][*:2].[*:3][*:2].[*:5]",
                   (1, 2),(1, 1, 1)),
    OperatorSmarts("Preparation of Sulfides, Intramolecular", 
                   "[C,c;+0:1]-@[SX2+0:5][C,c;+0:3].[F,Cl,Br,I;+0H:2]>>([*:1][*:2].[*:3][*:2]).[*:5]",
                   (1, 2),(1, 1),
                   kekulize_flag = True),
    # Thiols from Thiourea
    OperatorSmarts("Thiols from Thiourea", 
    ...
    OperatorSmarts("Thioacetals from Thiols and Aldehydes or Ketones", 
                   "[C,c;+0:1][SX2+0:2]-!@[C!$(*O)X4+0:5]-!@[SX2+0:4][C,c;+0:3].[O+0H2:6]>>[*:1][*:2].[*:3][*:4].[*:5]=[*:6]",
                   (1, 1),(1, 1, 1),
                   Retro_Not_Aromatic = True),     
    # Cyclic Thioacetals from Dithiols      
    OperatorSmarts("Cyclic Thioacetals from Dithiols", 
                   "[C,c;+0:1]-@[SX2+0:2][C!$(*O)X4+0:5][SX2+0:4]-@[C,c;+0:3].[O+0H2:6]>>([*:1][*:2].[*:3][*:4]).[*:5]=[*:6]",
                   (1, 1),(1, 1),
                   ring_issue = True,
                   Retro_Not_Aromatic = True),   
    # Oxidation of Thiols, Reduction of Disulfides     
    OperatorSmarts("Oxidation of Thiols", 
                   "[C,c;+0:1][SX2+0:2]-!@[SX2+0:4][C,c;+0:3].[O+0H2:5]>>[*:1][*:2].[*:3][*:4].[*:5]=[O]",
                   (1, 1),(1, 1, 0.5)),   
    OperatorSmarts("Oxidation of Thiols, Intramolecular", 
                   "[C,c;+0:1][SX2+0:2]-@[SX2+0:4][C,c;+0:3].[O+0H2:5]>>([*:1][*:2].[*:3][*:4]).[*:5]=[O]",
                   (1, 1),(1, 0.5)),     
    OperatorSmarts("Reduction of Disulfides", 
                   "[C,c;+0:1][SX2+0H:2].[C,c;+0:3][SX2+0H:4]>>[*:1][*:2][*:4][*:3].[H][H]",
                   (1, 1),(1, 1)),     
    OperatorSmarts("Reduction of Disulfides, Intramolecular", 
                   "([C,c;+0:1][SX2+0H:2].[C,c;+0:3][SX2+0H:4])>>[*:1][*:2][*:4][*:3].[H][H]",
                   (1, ),(1, 1)),  
    # Thiol-ene Reaction
    OperatorSmarts("Thiol-ene Reaction", 
                   "[C,c;+0:1][SX2+0:2]-!@[C+0:3][C+0!H0:4]>>[*:1][*:2].[*:3]=[*:4]",
                   (1, ),(1, 1),
                   Retro_Not_Aromatic = True),   
    OperatorSmarts("Thiol-ene Reaction, Intramolecular", 
                   "[C,c;+0:1][SX2+0:2]-@[C+0:3][C+0!H0:4]>>([*:1][*:2].[*:3]=[*:4])",
                   (1, ),(1, ),
                   Retro_Not_Aromatic = True),  
    # Sulfides Hydrogenolysis
    OperatorSmarts("Sulfides Hydrogenolysis", 
                   "[C,c;+0!H0:1].[C,c;+0!H0:3].[SX2+0H2:2]>>[*:1][*:2][*:3].[H][H]",
                   (1, 1, 1),(1, 2)),    
    OperatorSmarts("Sulfides Hydrogenolysis, Intramolecular",                  # works for aromatic ring
                   "([C,c;+0!H0:1].[C,c;+0!H0:3]).[SX2+0H2:2]>>[*:1][*:2][*:3].[H][H]",
                   (1, 1),(1, 2)),    
    # Oxidation of Disulfides
    OperatorSmarts("Oxidation of Disulfides", 
                   "[C,c;+0:1][SX4+0:2](=[O+0:5])(=[O+0:6])[O+0H:7].[C,c;+0:4][SX4+0:3](=[O+0])(=[O+0])[O+0H]>>[*:1][*:2][*:3][*:4].[*:5]=[*:6].[*:7]",
                   (1, 1),(1, 2.5, 1)),    
    OperatorSmarts("Oxidation of Disulfides, Intramolecular", 
                   "([C,c;+0:1][SX4+0:2](=[O+0:5])(=[O+0:6])[O+0H:7].[C,c;+0:4][SX4+0:3](=[O+0])(=[O+0])[O+0H])>>[*:1][*:2][*:3][*:4].[*:5]=[*:6].[*:7]",
                   (1, ),(1, 2.5, 1)),   
    # Cleavage of Disulfides by Halogens
    OperatorSmarts("Cleavage of Disulfides by Halogens", 
                   "[C,c;+0:1][SX2+0:2][F,Cl,Br,I;+0:5].[C,c;+0:4][SX2+0:3][F,Cl,Br,I;+0:6]>>[*:1][*:2][*:3][*:4].[*:5][*:6]",
                   (1, 1),(1, 1)),
    OperatorSmarts("Cleavage of Disulfides by Halogens, Intramolecular", 
                   "([C,c;+0:1][SX2+0:2][F,Cl,Br,I;+0:5].[C,c;+0:4][SX2+0:3][F,Cl,Br,I;+0:6])>>[*:1][*:2][*:3][*:4].[*:5][*:6]",
                   (1, ),(1, 1)),
    ...
    OperatorSmarts("Oxidation of Sulfides",                                    # works for S1C=CC=C1, retro aromatic ok
                   "[*+0:1][SX3!$(*[OH])+0:2](=[O+0:4])[*+0:3]>>[*:1][*:2][*:3].[*:4]=[O]",
                   (1, ),(1, 0.5)),   
    OperatorSmarts("Oxidation of Sulfoxides", 
                   "[*+0:1][SX4+0:2](=[O+0:4])(=[O+0:5])[*+0:3]>>[*:1][*:2](=[*:4])[*:3].[*:5]=[O]",
                   (1, ),(1, 0.5)),        
    # Deoxygenation of Sulfoxides 
    OperatorSmarts("Deoxygenation of Sulfoxides", 
                   "[C,c;+0:1][SX2+0:2][C,c;+0:3].[O+0H2:4]>>[*:1][*:2](=[*:4])[*:3].[H][H]",
                   (1, 1),(1, 1),
                   kekulize_flag = True),    
    # Thermal Elimination of Sulfoxides
    OperatorSmarts("Thermal Elimination of Sulfoxides", 
    ...
    OperatorSmarts("Thione-Thiol Tautomerization", 
                   "[N,O;+0:1]=[C+0:2][SX2+0H:3]>>[*:1][*:2]=[*:3]",  
                   (1, ),(1, ),
                   kekulize_flag = True),   
    OperatorSmarts("Thione-Thiol Tautomerization Reverse", 
    ...
    OperatorSmarts("Sulfenic Acid Tautomerization", 
                   "[C,c;+0:1][SX3+0H:2]=[O+0:3]>>[*:1][*:2][*:3]",
                   (1, ),(1, )),     
    # Sulfenic Acid Tautomerization
    OperatorSmarts("Sulfenic Acid Tautomerization Reverse", 
                   "[C,c;+0:1][SX2+0:2][O+0H:3]>>[*:1][*:2]=[*:3]",
                   (1, ),(1, )),    
    # Thiol Oxidation to Sulfenic Acid, Sulfinic Acid, Sulfonic Acid
    OperatorSmarts("Thiol Oxidation to Sulfenic Acid", 
                   "[C,c;+0:1][SX2+0:2][O+0H:3]>>[*:1][*:2].[*:3]=[O]",
                   (1, ),(1, 0.5)),   
    OperatorSmarts("Thiol Oxidation to Sulfinic Acid", 
                   "[C,c;!$(*=O)+0:1][SX3+0:2](=[O+0:3])[O+0H:4]>>[*:1][*:2].[*:3]=[*:4]",
                   (1, ),(1, 1)),    
    OperatorSmarts("Thiol Oxidation to Sulfonic Acid", 
                   "[C,c;!$(*=O)+0:1][SX4+0:2](=[O+0:3])(=[O+0])[O+0H:4]>>[*:1][*:2].[*:3]=[*:4]",
                   (1, ),(1, 1.5)),        
    # Sulfurous Acid Salts Addition to Oxiranes
    OperatorSmarts("Sulfurous Acid Salts Addition to Oxiranes", 
    ...
    OperatorSmarts("Sulfuric Acid Esterification, Acid Anhydride Formation", 
                   "[C,c;+0:1][O+0:2]-!@[SX4+0:4](=[O+0:3])(=[O+0:5])[OX2+0:6].[O+0H2:7]>>[*:1][*:2].[*:3]=[*:4](=[*:5])([*:6])[*:7]",
                   (1, 1),(1, 1)),
    OperatorSmarts("Organosulfates Hydrolysis", 
                   "[C,c;+0:1][O+0H:2].[O+0:3]=[SX4+0:4](=[O+0:5])([OX2+0:6])[O+0H:7]>>[*:1][*:2][*:4](=[*:3])(=[*:5])[*:6].[*:7]",
                   (1, 1),(1, 1)),          
    OperatorSmarts("Sulfuric Acid Esterification, Acid Anhydride Formation, Intramolecular", 
                   "[C,c;+0:1][O+0:2]-@[SX4+0:4](=[O+0:3])(=[O+0:5])[OX2+0:6].[O+0H2:7]>>([*:1][*:2].[*:3]=[*:4](=[*:5])([*:6])[*:7])",
                   (1, 1),(1, )),
    OperatorSmarts("Organosulfates Hydrolysis, Intramolecular", 
                   "([C,c;+0:1][O+0H:2].[O+0:3]=[SX4+0:4](=[O+0:5])([OX2+0:6])[O+0H:7])>>[*:1][*:2][*:4](=[*:3])(=[*:5])[*:6].[*:7]",
                   (1, ),(1, 1)), 
    # Organosulfates Reduction
    OperatorSmarts("Organosulfates Reduction", 
                   "[C!$(*=O)+0:1][SX4+0:3](=[O+0:4])(=[O+0:5])[O+0H:6].[O+0H2:2]>>[*:1][*:2][*:3](=[*:4])(=[*:5])[*:6].[H][H]",
                   (1, 1),(1, 1)),   
    # Alkenes Addition by Sulfuric Acid
    OperatorSmarts("Alkenes Addition by Sulfuric Acid", 
                   "[C+0!H0:1][C+0:2][O+0:6][SX4+0:4](=[O+0:3])(=[O+0:5])[O+0H:7]>>[*:1]=[*:2].[*:3]=[*:4](=[*:5])([*:6])[*:7]",
                   (1, ),(1, 1),
                   Retro_Not_Aromatic = True),     
    OperatorSmarts("Alkenes Addition by Sulfuric Acid Reverse", 
                   "[C+0:1]=[C+0:2].[O+0:3]=[SX4+0:4](=[O+0:5])([O+0H:6])[O+0H:7]>>[*:1][*:2][*:6][*:4](=[*:3])(=[*:5])[*:7]",
                   (1, 1),(1, ),
                   kekulize_flag = True), 
    # Alkenes Addition by Bisulfites
    OperatorSmarts("Alkenes Addition by Bisulfites", 
                   "[C+0!H0:1][C+0:2][SX4+0:4](=[O+0:3])(=[O+0:5])[O+0H:6]>>[*:1]=[*:2].[*:3]=[*:4]([*:5])[*:6]",
                   (1, ),(1, 1),
                   Retro_Not_Aromatic = True),            
    # Esterification of Sulfonic Acids, Sulfonic Acid Esters Hydrolysis
    OperatorSmarts("Esterification of Sulfonic Acids", 
                   "[C,c;+0:1][O+0:2]-!@[SX4+0:4](=[O+0:3])(=[O+0:5])[C,c,N;+0:6].[OH2,FH,ClH,BrH,IH;+0:7]>>[*:1][*:2].[*:3]=[*:4](=[*:5])([*:6])[*:7]",
                   (1, 1),(1, 1)),
    OperatorSmarts("Sulfonic Acid Esters Hydrolysis", 
                   "[C,c;+0:1][O+0H:2].[O+0:3]=[SX4+0:4](=[O+0:5])([C,c,N;+0:6])[OH+0:7]>>[*:1][*:2][*:4](=[*:3])(=[*:5])[*:6].[*:7]",
                   (1, 1),(1, 1)),   
    OperatorSmarts("Esterification of Sulfonic Acids, Intramolecular", 
                   "[C,c;+0:1][O+0:2]-@[SX4+0:4](=[O+0:3])(=[O+0:5])[C,c,N;+0:6].[OH2,FH,ClH,BrH,IH;+0:7]>>([*:1][*:2].[*:3]=[*:4](=[*:5])([*:6])[*:7])",
                   (1, 1),(1, )),
    OperatorSmarts("Sulfonic Acid Esters Hydrolysis, Intramolecular", 
                   "([C,c;+0:1][O+0H:2].[O+0:3]=[SX4+0:4](=[O+0:5])([C,c,N;+0:6])[OH+0:7])>>[*:1][*:2][*:4](=[*:3])(=[*:5])[*:6].[*:7]",
                   (1, ),(1, 1)),          
    OperatorSmarts("Sulfonic Esters Nucleophilic Substitution", 
                   "[C,c;+0:1][F,Cl,Br,I;+0:2].[O+0:3]=[SX4+0:4](=[O+0:5])([C,c;+0:6])[OH+0:7]>>[*:1][*:7][*:4](=[*:3])(=[*:5])[*:6].[*:2]",
                   (1, 1),(1, 1)),
    OperatorSmarts("Sulfonic Esters Nucleophilic Substitution, Intramolecular", 
                   "([C,c;+0:1][F,Cl,Br,I;+0:2].[O+0:3]=[SX4+0:4](=[O+0:5])([C,c;+0:6])[OH+0:7])>>[*:1][*:7][*:4](=[*:3])(=[*:5])[*:6].[*:2]",
                   (1, ),(1, 1)),
    # Sulfonation of Benzene    
    OperatorSmarts("Sulfonation of Benzene", 
