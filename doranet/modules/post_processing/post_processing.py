...
reaxys_not_supported = (       
    "O", "O=O", "[H][H]", "Br", "[Br][Br]", "Cl", "[Cl][Cl]",
    "O=S(O)O", "N", "O=S(=O)(O)O", "O=NO", "N#N", "O=[N+]([O-])O", "NO", "C#N", 
    "S", "O=S=O", "[C-]#[O+]", "O=C=O",
    )               
temp_ms = []
...
def pretreat_networks(total_generations, starters, helpers, network_forward_name = False, network_forward_obj = False, network_retro_name = False, network_retro_obj = False, job_name = "default_job_name", remove_pure_helpers_rxns = True, sanitize = True, calculate_dH = True, transform_enols = True):         
    '''
    ...
    if network_forward_name:    
        ...
        print("number of reations in network_forward", len(network_forward.rxns))
    ...
    def ngHCal(_mol_num, network_name):                                        # calculate Hf of all moleclues in network
    ...
    def RxnHCal (operatorNum, reas, pros, network_name, dirction, return_zero, rxn_type):             # calculate dH of all reactions in network and generate a string
        if dirction == "f":
            reactants_stoi = network_name.ops.meta(operatorNum, keys = ["reactants_stoi"])["reactants_stoi"]
            products_stoi = network_name.ops.meta(operatorNum, keys = ["products_stoi"])["products_stoi"]
        if dirction == "r":
            reactants_stoi = network_name.ops.meta(operatorNum, keys = ["products_stoi"])["products_stoi"]
            products_stoi = network_name.ops.meta(operatorNum, keys = ["reactants_stoi"])["reactants_stoi"]                      
        Hcorr = network_name.ops.meta(operatorNum, keys = ["enthalpy_correction"])["enthalpy_correction"]      
        dH = 0.0      
    ...
    if network_forward_name or network_forward_obj:
        for rxn in network_forward.rxns:                  
            ...
            if network_forward.ops.meta(operator, keys = ["Reaction_type"])["Reaction_type"] == "Enzymatic":
                dH = RxnHCal(operator, reactants, products, network_forward, "f", False, "Enzymatic")
            elif network_forward.ops.meta(operator, keys = ["Reaction_type"])["Reaction_type"] == "Catalytic":
                if calculate_dH:
                    dH = RxnHCal(operator, reactants, products, network_forward, "f", False, "Catalytic")
                else:
                    dH = RxnHCal(operator, reactants, products, network_forward, "f", True, "Catalytic")
            rxn_string = str()
            for i in reactants:                     
                rxn_string = rxn_string + network_forward.mols[i].uid + "."
            rxn_string = rxn_string[:-1]                    
            rxn_string = rxn_string + ">" + network_forward.ops.meta(operator, keys = ["name"])["name"] + ">" + str(dH) + ">"  
            for i in products:                      
                rxn_string = rxn_string + network_forward.mols[i].uid + "."
            rxn_string = rxn_string[:-1]
    ...
    if network_retro_name or network_retro_obj:
        for rxn in network_retro.rxns:        
            ...
            operator = rxn.operator
            if network_retro.ops.meta(operator, keys = ["Reaction_type"])["Reaction_type"] == "Enzymatic":
                dH = RxnHCal(operator, reactants, products, network_retro, "r", False, "Enzymatic")
            elif network_retro.ops.meta(operator, keys = ["Reaction_type"])["Reaction_type"] == "Catalytic":
                if calculate_dH:
                    dH = RxnHCal(operator, reactants, products, network_retro, "r", False, "Catalytic")  
                else:
                    dH = RxnHCal(operator, reactants, products, network_retro, "r", True, "Catalytic")
            rxn_string = str()
            for i in reactants:                     
                rxn_string = rxn_string + network_retro.mols[i].uid + "."
            rxn_string = rxn_string[:-1]                   
            rxn_string = rxn_string + ">" + network_retro.ops.meta(operator, keys = ["name"])["name"] + ">" + str(dH) + ">" 
            for i in products:                      
                rxn_string = rxn_string + network_retro.mols[i].uid + "."
            rxn_string = rxn_string[:-1]
            whole_rxns_list.append(rxn_string)
    ...
    def transform_enols(input_rxn_string):
        ...
        rxn = AllChem.ReactionFromSmarts("[C+0:1]=[C+0:2][O+0H:3]>>[*:1][*:2]=[*:3]")        
        pros_string = str()
        dH = 0
        for pro in pros:           
            pro_mol = Chem.MolFromSmiles(pro)                    
            if len(pro_mol.GetSubstructMatches(patt_enol)) == 1:  # ignore rare cases with multiple enol -OH
                comb = (pro_mol,)
                products_sets = rxn.RunReactants(comb)
                new_smiles = Chem.MolToSmiles(products_sets[0][0])
                if smilesHfCal(new_smiles, calculate_dH) is not None:   # new keto may not be supported by the enthalpy calculator           
                    pros_string = pros_string + new_smiles + "."
                    dH = dH + smilesHfCal(new_smiles, calculate_dH) - smilesHfCal(pro, calculate_dH)
                else:
    ...
    for i in data:
        ...
        if left != right and (helpers_reacting_flag is False or remove_pure_helpers_rxns is False): # and float(dH) < 0:    # set dH threshold if needed
    ...
    with open(f'{job_name}_network_pretreated.json', 'w', encoding = 'utf-8') as convert_file:
        convert_file.write(json.dumps(my_list))
    


def pathway_finder(starters, helpers, target, search_depth, max_num_rxns, min_rxn_atom_economy, job_name = "default_job_name", consider_name_difference = True): 
    '''
    ...
    f = open(f'{job_name}_network_pretreated.json', encoding = 'utf-8')  # open json file   
    data = json.load(f)
    f.close()    
    ...
    rxn_dict = dict()                            # { rxn_idx: {name: xx   reas: []  pros: []  mid: dH$()$()   } }
    rxn_idx = 0
    for rxn in data:
        pros = rxn.split(">")[3].split(".")
        reas = rxn.split(">")[0].split(".")
        ...
        rxn_dict[rxn_idx] = {"name": name, "reas": reas, "pros": pros, "mid": mid}
        rxn_idx += 1
    ...
    for rxn_idx in rxn_dict:                    # build consumers_dict, producers_dict for find_consumers(), find_producers()
        products = rxn_dict[rxn_idx]["pros"]    # also build atom econmy in producer_dict: {smiles: {rxn_idx: economy, rxn_idx2: economy ...}}
    ...
    print("searching for pathways. if it is taking too long, try adjusting pruning parameters")
    print()
    no_pathway = False
    while keep_expanding_flag is True:
        try:
            ...
            if first_path[0] == 0:                                               
                ...
                path_len = len(first_path[2:])              # number of rxns in current pathway                 
                for producer in producers:                  # int, index of rxn
                    if producers_dict[node][producer] > min_atom_economy:                       
                        ...
                        to_append.append(producer)     # add producer rxn to this pathway              
                        to_append[1] = to_append[1][1:]   # remove expended node                        
                        deadend_flag = False        
                        for rea in rxn_dict[producer]["reas"]:  # check if a lower gen mol already included                     
                            flag1 = True
                            if (node_gen + 1 == gen_limit and rea not in starters_set) or find_min_distance(rea) + node_gen + 1 > gen_limit:
                                deadend_flag = True
                                break                            
                            for node_idx2, t in enumerate(to_append[1]):
                                if t[0] == rea:
                                    to_append[1][node_idx2] = (rea, max(node_gen + 1, t[1]) )    # keep the higer gen number, not lower
                                    flag1 = False
                        ...
                        if deadend_flag is False:  
                            ...
                            for _nodes in to_append[1]:
                                Y_num_of_producers.append( find_producers_num(_nodes[0]) )  # sort nodes by its producers, faster
                            to_append[1] =  [x for _,x in sorted(zip(Y_num_of_producers,to_append[1]))]                       
                            if not to_append[1]:
                                to_append[0] = 1    # finished
                                work_list.append(to_append)                            
                            if to_append[1] and path_len + 1 < pathway_max_length:    
    ...
    def index_to_path(_rxn_idx):
        ...
        result_string = result_string + ">" + rxn_dict[_rxn_idx]["name"] + ">" + rxn_dict[_rxn_idx]["mid"] + ">"
    ...
    def find_cyc(_pathway, _starters, _helpers):  # return bool, if there're cycles in the pathway        
            G = nx.DiGraph()                
            rxn_node = 0                
            for idx, rxn in enumerate(_pathway["SMILES"]):                 # add nodes and edges to graph
                reas = rxn.split(">>")[0].split(".")
    ...
    if no_pathway is False:
        ...
        for path_list in to_save:
            ...
            for each_rxn_string in path_list:
                clean_smiles_list.append(each_rxn_string.split(">")[0] + ">>" + each_rxn_string.split(">")[3])
                clean_name_list.append(each_rxn_string.split(">")[1])
                clean_dH_list.append(each_rxn_string.split(">")[2].split("$")[0])
                #clean_dH_list.append(str(round(float(each_rxn_string.split(">")[2].split("$")[0]), 4)))
                stoi_list.append(each_rxn_string.split(">")[2].split("$")[1] + "$" + each_rxn_string.split(">")[2].split("$")[2])
        ...
        for path in pathways_list:                      
            if (consider_name_difference or path["SMILES"] not in path_rxnlist_set or path["enthalpy"] not in path_dH_set) and find_cyc(path, clean_starters, clean_helpers) is False:            
                ...
                f_result.write("place holder " + path["pathway_by-product"] + "\n")
                f_result.write("reaction SMILES stoichiometry " + str(path["stoi"]) + "\n")
        ...
        def SMILES_rm_keku(_SMILES):   # remove reaxys unsuported mols, kekulize mols, return a reaction SMILEs
            ...
            for rea in reas:
                if rea not in reaxys_not_supported and rea not in reas_set:
                    new_rxn = new_rxn + Chem.MolToSmiles(Chem.MolFromSmiles(rea), kekuleSmiles = True) + "."
                    reas_set.add(rea)
            new_rxn = new_rxn[:-1] + ">>"
            for pro in pros:
                if pro not in reaxys_not_supported and pro not in pros_set:
                    new_rxn = new_rxn + Chem.MolToSmiles(Chem.MolFromSmiles(pro), kekuleSmiles = True) + "."
                    pros_set.add(pro)
            if new_rxn[-1] == ".":
                new_rxn = new_rxn[:-1]
            return new_rxn
        ...
        if True:
            f_result = open(f'{job_name}_reaxys_batch_query.txt', "w", encoding = 'utf-8') 
            for rxn in rxn_set:           
                print_flag = True    
        ...
        if len(rxn_set) > max_rxns_per_batch: 
            ...
            filenames = [f'{job_name}_reaxys_batch_query_part_' + str(i) for i in range(1, num_files + 1)]            
            output_dict = dict()
            
            clean_list = list(rxn_set)
            for idx, filename in enumerate(filenames):
                if idx < num_files - 1:
                    output_dict[filename] = clean_list[idx * max_rxns_per_batch : (idx + 1) * max_rxns_per_batch]
                else:
                    output_dict[filename] = clean_list[idx * max_rxns_per_batch : ]
...
class MaxEnthalpyFilter(metadata.ReactionFilterBase):
    ...
    @property
    def meta_required(self) -> interfaces.MetaKeyPacket:
        return interfaces.MetaKeyPacket(molecule_keys={self.H_key}, operator_keys={'reactants_stoi',"products_stoi","enthalpy_correction","number_of_steps"})   
@typing.final
@dataclasses.dataclass(frozen=True)
...
def Inter_byproduct(path_idx, _intermediate, _smiles_list): # _intermediate included in _smiles_list
    my_num_gens = 1
    ...
    H_calc = EnthalpyCalculator("Enthalpy_f")
    thermo_filter = MaxEnthalpyFilter(15, "Enthalpy_f")
    reaction_plan = No_helpers_reaction_Filter(_intermediate) >> Ring_Issues_Filter() >> H_calc >> thermo_filter
...
# pathway ranking        
def pathway_ranking(starters, helpers, target, weights, num_process, reaxys_result_name = False, job_name = "default_job_name", cool_reactions = False):                 
    ...
    for idx, marker in enumerate(pathway_marker):
        ...
        temp_dict["SMILES"] = clean_list[marker + 6 : marker + 6 + int((next_marker - (marker + 6))/3)  ]    
        temp_dict["name"] = clean_list[marker + 6 + int((next_marker - (marker + 6))/3) : marker + 6 + int((next_marker - (marker + 6))/3) * 2 ] 
        temp_dict["enthalpy"] = clean_list[marker + 6 + int((next_marker - (marker + 6))/3) * 2 : marker + 6 + int((next_marker - (marker + 6))/3) * 3 ]               
        pathways_list.append(temp_dict)
    ...
    for record in pathways_list:
        ...
        for idx, i in enumerate(record["SMILES"]):
            rxn_string = i.split(">>")[0] + ">" + record["name"][idx] + ">" + record["enthalpy"][idx] + "$" + record["stoi"][idx] + ">" + i.split(">>")[1]
    ...
    data = temp_data   
    print("Start ranking pathways. Number of pathways after sanitization:", len(data))
    ...
    def path_eco (_path):      # assuming no circular loops; if a middle rxn produces a starter or intermediat consumed in upstream, it's considered recycled;  
        _path = list(_path)
      ...
      #  target_mol = float()        
        idx_remove = 0
        for idx, rxn in enumerate(_path):    # find rxn for final target, add reas in left, by-product in right, remove rxn
            pros = rxn.split(">")[3].split(".")
            reas = rxn.split(">")[0].split(".")
                ...
                for idx2, pro in enumerate(pros):
                    if pro != target_smiles:
                        if pro not in right_dict:
                            ...
                            right_dict[pro] = right_dict[pro] + pro_stoi[idx2]/target_stoi
                for idx2, rea in enumerate(reas):
                    if rea not in left_dict:
                        left_dict[rea] = rea_stoi[idx2]/target_stoi                       
                    else:
                        left_dict[rea] = left_dict[rea] + rea_stoi[idx2]/target_stoi                   
                break
        ...
        timecount = 0
        while repeat_flag is True and len(_path) > 0:    # repeat until left only contain starters; check if all parents are starters
            timecount += 1
            popkey = 99
            for i in left_dict:       # find idx for a intermediate molecule to produce
                ...
                if popkey in pros:
                   # explored_rxns.add(rxn)
                    ...
                    for idx2, pro in enumerate(pros):
                        ...
                        if pro not in right_dict:                                
                            right_dict[pro] = num_mol / popmol_stoi * pro_stoi[idx2]
                        else:
                            right_dict[pro] = right_dict[pro] + num_mol / popmol_stoi * pro_stoi[idx2]
                    for idx2, rea in enumerate(reas):
                        if rea not in left_dict:
                            left_dict[rea] = num_mol / popmol_stoi * rea_stoi[idx2]                       
                        else:
                            left_dict[rea] = left_dict[rea] + num_mol / popmol_stoi * rea_stoi[idx2]  
        ...
        by_product_weight = 0
        for i in right_dict:            
            by_product_weight += Descriptors.MolWt(Chem.MolFromSmiles(i)) * right_dict[i]            
    ...
    if weights["by_product_number"] != 0: 
        ...
        with Pool(processes = num_process) as pool:
            results = [pool.apply_async(Byproduct_index, args = (work[0], work[1])) for work in soup_list]    # workers pool
            by_pro_list_tuples = [r.get() for r in results]    # [(path_idx, byproduct num),]    
            
        with Pool(processes = num_process) as pool:
            results = [pool.apply_async(Inter_byproduct, args = (work[0], work[1], work[2])) for work in Inter_soup_list]    # workers pool
            inter_by_pro_list_tuples = [r.get() for r in results]    # [(path_idx, SMILES, inter-byproduct num),]    
    ...
    if weights["in_reaxys"] != 0: 
        ...
        # load 2 files
        reaxys_batch = np.genfromtxt(f'{job_name}_reaxys_batch_query.txt', comments = '?', dtype = str, delimiter=",", skip_header=0 )
        reaxys_result = np.genfromtxt(reaxys_result_name, comments = '?', dtype = int, delimiter=",", skip_header=0, usecols = (2) )
    ...
    for i in range(len(data)):
        ...
        final_score.append( H_score_list[i] * weight[0] + step_score_list[i] * weight[1]  + by_pro_score_list[i] * weight[2] + eco_score_list[i] * weight[3] + salt_score_list[i] * weight[4] + reaxys_score_list[i] *  weight[5] + cool_score_list[i] * weight[6])
    ...
    final_score, data, eco_list, by_pro_list, intermediate_by_product_dict_list,  H_score_list, step_score_list, by_pro_score_list, eco_score_list, salt_score_list, reaxys_score_list, cool_score_list  = zip(*sorted(zip(final_score, data, eco_list, by_pro_list, intermediate_by_product_dict_list, H_score_list, step_score_list, by_pro_score_list, eco_score_list, salt_score_list, reaxys_score_list, cool_score_list)))

    def clean_rxn_strings(_path):  # for a path, return a list of clean rxn smiles, names, dH of each rxn
        clean_path = list()
        names = list()
        dH_list = list()
        for rxn in _path:
    ...
    for idx, path in reversed(list(enumerate(data))):           # print result
        ...
        if print_flag is True:
            ...
            print("Max reaction enthalpy score", H_score_list[idx], " x ", weight[0], " = ", H_score_list[idx] * weight[0])
            print("Number of reactions score", step_score_list[idx], " x ", weight[1], " = ", step_score_list[idx] * weight[1])
            print("By-product score", by_pro_score_list[idx], " x ", weight[2], " = ", by_pro_score_list[idx] * weight[2])
            print("Pathway atom economy score", eco_score_list[idx], " x ", weight[3], " = ", eco_score_list[idx] * weight[3])
            print("Salt score", salt_score_list[idx], " x ", weight[4], " = ", salt_score_list[idx] * weight[4])
            print("Reaxys score", reaxys_score_list[idx], " x ", weight[5], " = ", reaxys_score_list[idx] * weight[5])
            print("Cool score", cool_score_list[idx], " x ", weight[6], " = ", cool_score_list[idx] * weight[6])
            ...
            f_result.write("\n")
            print("intermediate by-product", intermediate_by_product_dict_list[idx])
            f_result.write("intermediate by-product " + str(intermediate_by_product_dict_list[idx]))
            f_result.write("\n")
            f_result.write("reaction SMILES, name, and enthalpy:" )
            f_result.write("\n")
...
def create_page(pathway_dict, page_number, reaxys_result_set, _my_start, _helpers, _reaxys_rxn_color, _normal_rxn_color):  
    ...
    for idx, i in enumerate(name_list): 
        name_list[idx] = textwrap.fill(name_list[idx], 25)   # wrape text so not too long
    
    title = "Rank " + pathway_dict["ranking"] + "\n" + "Atomic Economy " + pathway_dict["atomic_economy"] + "\n" + "Pathway by-product " + pathway_dict["pathway_by-product"]    # to display on each page
    ...
    G = nx.DiGraph(rankdir = "TB",  # top to bottom layout
                  # ranksep="0.8",                  
                   )
    
    inter_py_pro = pathway_dict["intermediate_by-product"]  # intermediate by product number, dict, key smiles, value int
    inter_py_pro["O=C=O"] = ""     # for reagent node (CO2)
    rxn_node = 0
    node_labels_dict = dict()
    node_labels_reagent = dict()
    
    reaxys_rxn_nodes = set()  # set of int, rxn nodes in reaxys_result_set, used for coloring edges
    ...
    for idx, rxn in enumerate(rxn_list):                 # add nodes and edges to graph
        ...
        G.add_node(rxn_node)
        if SMILES_rm_keku(rxn) in reaxys_result_set or ' '.join(name_list[idx].replace('\n', ' ').split()[:-2]) in known_rxn_set:
        ...
        for rea in reas:     # add nodes and edges for non-reagents
            if rea not in help_dict:                   
                G.add_node(rea, image1 = add_text(add_margin(trim(addjust_mol_image(Chem.MolFromSmiles(rea)))), str(inter_py_pro[rea])))
                G.add_edge(rea, rxn_node, arror_margin = 10)
                for pro in pros:        
                    if pro not in help_dict:                        
                        G.add_node(pro, image1 = add_text(add_margin(trim(addjust_mol_image(Chem.MolFromSmiles(pro)))), str(inter_py_pro[pro])))
    ...
    for i in my_start:
        if i in G.nodes:
            my_start_being_produced = False
            starter_index = 0                            # if starter usded multiple times, make them unique nodes
            for node in G.nodes:
                ...
                if i in list(G.predecessors(node)):
                    ...
                    G2.add_node(i + str(starter_index), image1 = add_text(add_margin(trim(addjust_mol_image(Chem.MolFromSmiles(i)))), str(inter_py_pro[i])))        
                    G2.add_edge(i + str(starter_index), node,  arror_margin = 10
                               #minlen = 2                      
                               )        
                    starter_index += 1  
    ...
    label_props = dict(boxstyle="round,pad=0.3", 
                       fc="white", 
                       ec="white", 
                     #  lw=1, 
                       )
...
def pathway_visualization(starters, helpers, num_process, reaxys_result_name = False, job_name = "default_job_name", exclude_smiles = [], reaxys_rxn_color = "blue", normal_rxn_color = "black"):            
    ...
    for idx, marker in enumerate(pathway_marker):
        ...
        temp_dict["atomic_economy"] = str("%.1f"%float(float(clean_list[marker + 2][15:])*100)) + "%"
    ...
    with Pool(processes = num_process) as pool:
        results = [pool.apply_async(create_page, args = (work[0], work[1], reaxys_set, my_start, help_dict, reaxys_rxn_color, normal_rxn_color)) for work in work_list]    # workers pool
